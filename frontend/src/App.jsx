...
function SettingsPage({ userProfile = {}, userAccounts = [], onSaveUserProfile, onSaveUserAccounts, onPersistUserProfile = null, authenticatedUser = null, authState = null, authMode = 'managed', cognitoConfig = null }) {
  ...
  async function handleSave() {
    ...
    if (typeof onPersistUserProfile === 'function') {
      try {
        const persistedProfile = await onPersistUserProfile({
        ...
        if (persistedProfile) {
          nextProfile = {
            ...nextProfile,
            nickname: String(persistedProfile.nickname || '').trim(),
            photoDataUrl: String(persistedProfile.photoDataUrl || '').trim(),
          };
        }
      } catch (error) {
        setImageError(error instanceof Error ? error.message : 'Profile could not be saved.');
        return;
      }
    }
  ...
  return (
    <div style={{ maxWidth: '860px', margin: '0 auto', display: 'grid', gap: '24px' }}>
      ...
      <section style={{ ...naesCardStyle(false), padding: '24px' }}>
        ...
        <div style={{ marginTop: '10px', display: 'grid' }}>
          {readOnlyRow('Auth Mode', String(authMode || 'managed'))}
          ...
          {readOnlyRow('Cognito Region', String(cognitoConfig?.region || '—'))}
          {readOnlyRow('Cognito User Pool ID', String(cognitoConfig?.userPoolId || '—'))}
          {readOnlyRow('Cognito App Client ID', String(cognitoConfig?.appClientId || '—'))}
          {readOnlyRow('Cognito Domain', String(cognitoConfig?.domain || '—'))}
          {readOnlyRow('Cognito Redirect Sign In', String(cognitoConfig?.redirectSignIn || '—'))}
          {readOnlyRow('Cognito Redirect Sign Out', String(cognitoConfig?.redirectSignOut || '—'))}
        </div>
      </section>
...
function buildUserApiPayload(draftUser = {}, fallbackUser = {}) {
  const sourceFullName = String(draftUser.fullName || fallbackUser.fullName || '').trim();
  const split = splitFullName(sourceFullName);

  return {
    email: String(draftUser.email || fallbackUser.email || '').trim().toLowerCase(),
    firstName: split.firstName || String(draftUser.firstName || fallbackUser.firstName || '').trim(),
    lastName: split.lastName || String(draftUser.lastName || fallbackUser.lastName || '').trim(),
    nickname: String(draftUser.nickname || fallbackUser.nickname || '').trim() || null,
    title: String(draftUser.title || fallbackUser.title || '').trim() || null,
    role: frontendUserRoleToBackend(draftUser.role || fallbackUser.role || 'Associate'),
    teamName: String(draftUser.team || draftUser.teamName || fallbackUser.team || fallbackUser.teamName || '').trim() || null,
    profilePhotoUrl: String(draftUser.photoDataUrl || fallbackUser.photoDataUrl || '').trim() || null,
    isActive: String(draftUser.status || fallbackUser.status || 'Active').trim().toLowerCase() !== 'inactive',
  };
}

function mapBackendUserToFrontendUserAccount(row = {}, draftUser = {}) {
  const fullName = [row.firstName, row.lastName].filter(Boolean).join(' ').trim();
  return buildUserAccountDraft({
    ...draftUser,
    id: String(row.id || draftUser.id || ''),
    fullName: fullName || String(draftUser.fullName || '').trim(),
    email: String(row.email || draftUser.email || '').trim(),
    nickname: String(row.nickname || draftUser.nickname || '').trim(),
    title: String(row.title || draftUser.title || '').trim(),
    role: backendUserRoleToFrontend(row.role || draftUser.role || ''),
    team: String(row.teamName || draftUser.team || draftUser.teamName || '').trim(),
    teamName: String(row.teamName || draftUser.team || draftUser.teamName || '').trim(),
    status: row.isActive === false ? 'Inactive' : 'Active',
    photoDataUrl: String(row.profilePhotoUrl || draftUser.photoDataUrl || '').trim(),
    linkedProfile: Boolean(draftUser.linkedProfile),
    _backend: row
  });
}

async function createUserViaApi(draftUser = {}) {
  const response = await backendFetch('/api/users', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(buildUserApiPayload(draftUser)),
  });

  const payload = await response.json().catch(() => null);

  if (!response.ok) {
    throw new Error(payload?.error || `User create failed: ${response.status}`);
  }

  return mapBackendUserToFrontendUserAccount(payload?.data || {}, draftUser);
}

async function updateUserViaApi(userId, draftUser = {}, fallbackUser = {}) {
  const response = await backendFetch(`/api/users/${userId}`, {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(buildUserApiPayload(draftUser, fallbackUser)),
  });

  const payload = await response.json().catch(() => null);

  if (!response.ok) {
    throw new Error(payload?.error || `User update failed: ${response.status}`);
  }

  return mapBackendUserToFrontendUserAccount(payload?.data || {}, draftUser);
}

function UserAccountsPage({ userAccounts = [], onSaveUserAccounts, onSyncCurrentUserProfile, onCreateUserAccount = null, onUpdateUserAccount = null }) {
  ...
  async function handleSave() {
    if (isCreatingNew) {
      ...
      try {
        if (typeof onCreateUserAccount === 'function') {
          nextUser = await onCreateUserAccount(draftUser);
        } else {
        }
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error || 'unknown_error');
        console.warn('Could not create user in backend, using local fallback', error);
      }
    ...
    let nextUpdatedRecord = {
      ...selectedRecord,
      ...draftUser
    };


    if (selectedRecord.id && !String(selectedRecord.id).startsWith('user-')) {
      try {
        if (typeof onUpdateUserAccount === 'function') {
          nextUpdatedRecord = await onUpdateUserAccount(selectedRecord.id, draftUser, selectedRecord);
        } else {
        }
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error || 'unknown_error');
        console.warn('Could not update user in backend, using local fallback', error);
      }
    }
    ...
    if (draftUser.linkedProfile) {
      const nextProfile = {
        fullName: nextUpdatedRecord.fullName,
        nickname: nextUpdatedRecord.nickname,
        title: nextUpdatedRecord.title,
        email: nextUpdatedRecord.email,
        role: nextUpdatedRecord.role,
        department: nextUpdatedRecord.department,
        team: nextUpdatedRecord.team,
        status: nextUpdatedRecord.status,
        permissionLevel: nextUpdatedRecord.permissionLevel,
        businessUnitAccess: nextUpdatedRecord.businessUnitAccess,
        serviceLineAccess: nextUpdatedRecord.serviceLineAccess,
        territoryScope: nextUpdatedRecord.territoryScope,
        authType: nextUpdatedRecord.authType,
        passwordNote: nextUpdatedRecord.passwordNote,
        lastLogin: nextUpdatedRecord.lastLogin,
        photoDataUrl: nextUpdatedRecord.photoDataUrl
      };
...
function OpportunityDetailPage({ onBackToOverview, opportunity, onSaveOpportunity, tasks = [], activities = [] }) {
  ...
  const relatedActivities = Array.isArray(activities)
    ? activities.filter((activity) => String(activity?.opportunityId || '') === String(opportunity?.id || ''))
    : [];
  ...
  const arrValue = Number(opportunity?.calc_arr_total ?? annualValue) || 0;
  ...
  const weightedValue = Number(
    opportunity?.weighted_revenue ??
    (totalEstimatedValue * ((Number(opportunity?.probability || 0) || 0) / 100))
  ) || 0;
  ...
  const summaryCards = [
    ['Estimated Year 1', money(annualValue)],
    ['ARR', money(arrValue)],
    ['Total Estimated', money(totalEstimatedValue)],
    ['Weighted Revenue', money(weightedValue)]
  ];

  const relationshipRows = [
    ['Linked Account', opportunity?.account || opportunity?.account_name || 'Not linked'],
    ['Primary Contact', opportunity?.primary_contact_name || 'No primary contact'],
    ['Account Owner', opportunity?.owner_full_name || 'Unassigned'],
    ['Team', opportunity?.owner_team_name || 'Not assigned']
  ];

  const storyRows = [
    ['Service Line', opportunity?.service_line || 'Not set'],
    ['Opportunity Type', opportunity?.opportunity_type || 'Not set'],
    ['Market Segment', opportunity?.market_segment || 'Not set'],
    ['Commercial Basis', opportunity?.commercial_basis || 'Imported record'],
  ...
  const governanceRows = [
    ...
    ['Forecast Category', opportunity?.forecast_category || 'Not set'],
    ['Expected Close', opportunity?.expected_close_date || 'Not set'],
    ['Last Activity', opportunity?.last_activity_date || 'No activity yet'],
    ['Staleness', opportunity?.staleness_flag || 'Unknown'],
    ['Forecast Hygiene', opportunity?.forecast_hygiene_status || 'Not set']
  ];
  ...
  const commercialRows = [
    ...
    ['Market Segment', opportunity?.market_segment || 'Not set'],
    ['Commercial Basis', opportunity?.commercial_basis || 'Imported record'],
    ['CTS %', `${Number(opportunity?.estimated_cts_pct || 0)}%`],
    ['Earnings %', `${Number(opportunity?.estimated_earnings_pct || 0)}%`]
  ];
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: '20px', flexWrap: 'wrap', alignItems: 'flex-start' }}>
          <div>
            ...
            <div style={{ marginTop: '8px', display: 'flex', gap: '8px', flexWrap: 'wrap', alignItems: 'center' }}>
              ...
              <span style={{ borderRadius: '999px', background: '#F2F5F2', padding: '4px 12px', fontSize: '12px', fontWeight: 700, color: '#475569' }}>
                {opportunity?.stage || 'Not set'}
              </span>
            </div>

            <p style={{ marginTop: '12px', maxWidth: '760px', fontSize: '14px', lineHeight: 1.6, color: '#475569' }}>
              {(opportunity?.name || 'Opportunity') + ' detail view based on the saved CRM record and related data.'}
            </p>
          </div>
      ...
      <section style={{ display: 'grid', gap: '24px', gridTemplateColumns: 'repeat(auto-fit, minmax(360px, 1fr))' }}>
              ...
              {relatedTasks.length ? (
                <div style={{ display: 'grid', gap: '12px' }}>
                  {relatedTasks.slice(0, 8).map((task) => (
                    <div key={task.id} style={{ borderRadius: '18px', border: '1px solid #E5ECE5', background: '#FBFCFB', padding: '14px 16px', display: 'flex', justifyContent: 'space-between', gap: '12px', alignItems: 'center' }}>
                      <div>
                        <div style={{ fontSize: '14px', fontWeight: 600, color: '#1f2937' }}>{task.title || 'Task'}</div>
                        <div style={{ marginTop: '4px', fontSize: '13px', color: '#64748b' }}>
                          {task.dueDate ? `Due ${task.dueDate}` : 'No due date'}
                        </div>
                      </div>
                      <div style={{ borderRadius: '999px', background: '#EEF4EE', color: '#45624C', padding: '4px 10px', fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.12em' }}>
              ...
              {relatedActivities.length ? (
                <div style={{ display: 'grid', gap: '12px' }}>
                  {relatedActivities.slice(0, 8).map((activity) => (
                    <div key={activity.id} style={{ borderRadius: '18px', border: '1px solid #E5ECE5', background: '#FBFCFB', padding: '14px 16px' }}>
                      <div style={{ fontSize: '14px', fontWeight: 600, color: '#1f2937' }}>
                        {activity.subject || activity.activityType || 'Activity'}
                      </div>
                      <div style={{ marginTop: '4px', fontSize: '13px', color: '#64748b', lineHeight: 1.5 }}>
                        {activity.notes || activity.activityDateTime || 'No activity notes'}
                      </div>
                    </div>
                  ))}
                </div>
        ...
        <div style={{ display: 'grid', gap: '24px' }}>
          <section style={shellCard}>
            ...
            <div style={{ marginTop: '16px', borderRadius: '18px', border: '1px solid #E5ECE5', background: '#FBFCFB', padding: '16px' }}>
              ...
              <div style={{ marginTop: '8px', fontSize: '14px', lineHeight: 1.65, color: '#475569' }}>
                {opportunity?.service_line || 'Opportunity'} opportunity for {opportunity?.account || opportunity?.account_name || 'unlinked account'}.
                {opportunity?.market_segment ? ` Market segment: ${opportunity.market_segment}.` : ''}
                {opportunity?.primary_contact_name ? ` Primary contact: ${opportunity.primary_contact_name}.` : ''}
                {opportunity?.expected_close_date ? ` Expected close: ${opportunity.expected_close_date}.` : ''}
              </div>
            </div>
          </section>
          ...
          <section style={shellCard}>
            ...
            <h3 style={{ margin: '8px 0 0 0', fontSize: '18px', fontWeight: 600 }}>Imported revenue profile</h3>
            <div style={{ marginTop: '18px', display: 'grid', gap: '14px', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))' }}>
              {[
                ['Annual', money(annualValue)],
                ['ARR', money(arrValue)],
                ['Total Estimated', money(totalEstimatedValue)],
                ['Weighted', money(weightedValue)]
              ].map(([label, value]) => (
                <div key={label} style={{ borderRadius: '18px', border: '1px solid #E5ECE5', background: '#FBFCFB', padding: '16px' }}>
                  <div style={{ fontSize: '10px', fontWeight: 700, letterSpacing: '0.18em', textTransform: 'uppercase', color: '#64748b' }}>{label}</div>
                  <div style={{ marginTop: '8px', fontSize: '22px', fontWeight: 700, color: '#111827' }}>{value}</div>
                </div>
              ))}
            </div>
...
export default function App() {
  ...
  const [accountList, setAccountList] = useState(() => sanitizeAccounts([]));
  ...
  const [sessionHydrationState, setSessionHydrationState] = useState({
    checked: false,
    loading: false,
  });
  const [cognitoCallbackState, setCognitoCallbackState] = useState({
    code: '',
    error: '',
    errorDescription: '',
    exchangeStatus: '',
  ...
  const [cognitoCrmUserState, setCognitoCrmUserState] = useState({
    status: '',
    error: '',
    payload: null,
  });

  const cognitoAuthenticatedIdentity = useMemo(() => {
    const idClaims = cognitoCallbackState?.exchangePayload?.data?.id_claims || null;
    const accessClaims = cognitoCallbackState?.exchangePayload?.data?.access_claims || null;

    if (!idClaims && !accessClaims) return null;

    return {
      email: String(idClaims?.email || '').trim(),
      sub: String(idClaims?.sub || accessClaims?.sub || '').trim(),
      name: String(idClaims?.name || '').trim(),
      username: String(accessClaims?.username || idClaims?.['cognito:username'] || '').trim(),
      tokenType: String(cognitoCallbackState?.exchangePayload?.data?.token_type || '').trim(),
      exchangeStatus: String(cognitoCallbackState?.exchangeStatus || '').trim(),
    };
  }, [cognitoCallbackState]);

  const API_BASE_URL = (import.meta.env.VITE_API_BASE_URL || '/api').replace(/\/$/, '');
  const AUTH_MODE = String(import.meta.env.VITE_AUTH_MODE || 'managed').trim().toLowerCase();
  const COGNITO_ENABLED = AUTH_MODE === 'cognito';
  const COGNITO_REGION = String(import.meta.env.VITE_COGNITO_REGION || '').trim();
  const COGNITO_USER_POOL_ID = String(import.meta.env.VITE_COGNITO_USER_POOL_ID || '').trim();
  const COGNITO_APP_CLIENT_ID = String(import.meta.env.VITE_COGNITO_APP_CLIENT_ID || '').trim();
  const COGNITO_DOMAIN = String(import.meta.env.VITE_COGNITO_DOMAIN || '').trim();
  const COGNITO_REDIRECT_SIGN_IN = String(import.meta.env.VITE_COGNITO_REDIRECT_SIGN_IN || '').trim();
  const COGNITO_REDIRECT_SIGN_OUT = String(import.meta.env.VITE_COGNITO_REDIRECT_SIGN_OUT || '').trim();
  ...
  function buildAuthRequestHeaders(extraHeaders = {}) {
    if (COGNITO_ENABLED) {
      return {
        ...extraHeaders,
      };
    ...
    return buildBackendHeaders(effectiveDevHeaderUserId, extraHeaders);
  }

  function buildCognitoAuthorizeUrl() {
    if (!COGNITO_DOMAIN || !COGNITO_APP_CLIENT_ID || !COGNITO_REDIRECT_SIGN_IN) {
      return '';
    }

    const base = `https://${COGNITO_DOMAIN}/login`;
  ...
  function readCognitoCallbackParams() {
    try {
  ...
  async function exchangeCognitoCode(code) {
    ...
    console.log('[frontend] exchangeCognitoCode start', {
      codePrefix: String(code || '').trim().slice(0, 12),
      url: buildBackendUrl('/api/auth/cognito/exchange'),
    });
    ...
    try {
      ...
      if (error?.name === 'AbortError') {
        throw new Error('exchange_timeout_after_8s');
      }
      throw error;
    }
    ...
    console.log('[frontend] exchangeCognitoCode response', {
      ok: response?.ok,
    ...
    console.log('[frontend] exchangeCognitoCode payload', {
      ok: response?.ok,
      status: response?.status,
      payload,
    });
    ...
    if (!response.ok) {
      throw new Error(payload?.error || `exchange_failed_${response.status}`);
    }

    return payload;
  }

  async function mapCognitoUserToCrmUser(identity = {}) {
    const controller = new AbortController();
    const timeoutId = window.setTimeout(() => controller.abort(), 8000);

    console.log('[frontend] mapCognitoUserToCrmUser start', {
      email: String(identity?.email || '').trim(),
      sub: String(identity?.sub || '').trim(),
      url: buildBackendUrl('/api/auth/cognito/map-user'),
    });
    ...
    try {
      ...
      window.clearTimeout(timeoutId);
      if (error?.name === 'AbortError') {
        throw new Error('crm_map_timeout_after_8s');
      }
      throw error;
    }
    ...
    console.log('[frontend] mapCognitoUserToCrmUser payload', {
      ok: response?.ok,
      status: response?.status,
      payload,
    });
    ...
    if (!response.ok) {
      throw new Error(payload?.error || `crm_user_map_failed_${response.status}`);
    }
  ...
  useEffect(() => {
    ...
    if (!cognitoCallbackState?.code) return;
    if (cognitoCallbackState?.exchangeStatus === 'loading') return;
    if (cognitoCallbackState?.exchangeStatus === 'success') return;
    if (cognitoCallbackState?.exchangeStatus === 'error') return;

    let cancelled = false;
    (async () => {
      try {
        setCognitoCallbackState((prev) => ({
          ...prev,
          exchangeStatus: 'loading',
          exchangeError: '',
        }));

        const exchangePayload = await exchangeCognitoCode(cognitoCallbackState.code);
        if (cancelled) return;

        const idClaims = exchangePayload?.data?.id_claims || {};
        const identity = {
          email: String(idClaims?.email || '').trim(),
          sub: String(idClaims?.sub || '').trim(),
          name: String(idClaims?.name || '').trim(),
          username: String(idClaims['cognito:username'] || '').trim(),
        };

        console.log('[frontend] post-exchange identity', {
          idClaims,
          identity,
        });
        ...
        if (!identity.email) {
          console.log('[frontend] post-exchange missing email', { idClaims, identity });
          throw new Error('missing_identity_email');
        }

        console.log('[frontend] about to start crm map', { identity });

        setCognitoCrmUserState({
          status: 'loading',
          error: '',
          payload: null,
        });

        const crmPayload = await mapCognitoUserToCrmUser(identity);
        if (cancelled) return;
        ...
        if (cancelled) return;
        console.log('[frontend] callback effect catch', {
          error: String(error?.message || error || 'cognito_callback_failed'),
          stack: error?.stack || null,
        });
        const message = String(error?.message || error || 'cognito_callback_failed');
        setCognitoCallbackState((prev) => ({
          ...prev,
          exchangeStatus: 'error',
          exchangeError: message,
        }));
        setCognitoCrmUserState((prev) => ({
          ...prev,
          status: 'error',
          error: message,
        }));
  ...
  }, [COGNITO_ENABLED, cognitoCallbackState?.code]);

  async function backendFetch(path = '', options = {}) {
    return fetch(buildBackendUrl(path), {
      ...options,
      credentials: 'include',
      headers: buildAuthRequestHeaders(options.headers || {}),
    });
  }
  ...
  async function createUserViaApiInApp(draftUser = {}) {
    const response = await backendFetch('/api/users', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(buildUserApiPayload(draftUser)),
  ...
  async function updateUserViaApiInApp(userId, draftUser = {}, fallbackUser = {}) {
    const response = await backendFetch(`/api/users/${userId}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(buildUserApiPayload(draftUser, fallbackUser)),
  ...
  const effectiveDevHeaderUserId = authenticatedCurrentUser?.id || selectedDevUser?.headerUserId || 'cmnf1dl3q0001itfekw0gh3nh';
  const effectiveUserRole = authenticatedCurrentUser?.role || selectedDevUser?.role || userProfile?.role || 'Associate';

  async function persistUserProfileToApi(updates = {}) {
    const currentUserId = String(authenticatedCurrentUser?.id || '').trim();
    if (!currentUserId) {
      throw new Error('No authenticated user available for profile save');
    }

    const response = await backendFetch(`/api/profile/${currentUserId}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        nickname: updates.nickname ?? null,
        profilePhotoUrl: updates.photoDataUrl ?? null,
      }),
    });
    ...
    if (!response.ok) {
      throw new Error(payload?.error || `Profile save failed: ${response.status}`);
    }
    ...
    const nextProfile = {
      ...buildDefaultUserProfile(),
      ...userProfile,
      fullName: [row.firstName, row.lastName].filter(Boolean).join(' ').trim() || userProfile.fullName,
      nickname: String(row.nickname || '').trim(),
      title: String(row.title || userProfile.title || '').trim(),
      email: String(row.email || userProfile.email || '').trim(),
      role: String(row.role || userProfile.role || '').trim(),
      team: String(row.teamName || userProfile.team || '').trim(),
      status: row.isActive === false ? 'Inactive' : 'Active',
      authType: COGNITO_ENABLED ? 'Cognito' : (userProfile.authType || 'Managed Identity'),
      photoDataUrl: String(row.profilePhotoUrl || '').trim(),
    };
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const crmUser = payload?.data || null;
        if (response.ok && crmUser?.id) {
          setAuthState({
            status: 'ready',
            currentUser: crmUser,
            error: '',
  ...
  useEffect(() => {
    if (!COGNITO_ENABLED) return;
    setCognitoCallbackState(readCognitoCallbackParams());
  }, [COGNITO_ENABLED, routePath]);

  useEffect(() => {
    if (!COGNITO_ENABLED) return;
    if (!sessionHydrationState.checked) return;

    const callbackState = readCognitoCallbackParams();
    const hasCallback = Boolean(callbackState?.code || callbackState?.error);
    const isMapped = Boolean(cognitoCrmUserState?.payload?.data?.mapped && cognitoCrmUserState?.payload?.data?.crmUser?.id);
  ...
  }, [COGNITO_ENABLED, routePath, cognitoCrmUserState, sessionHydrationState.checked]);
  ...
  useEffect(() => {
    ...
    const crmUser = cognitoCrmUserState?.payload?.data?.crmUser;
    const isMapped = Boolean(cognitoCrmUserState?.payload?.data?.mapped && crmUser?.id);
    ...
    setUserProfile((prev) => ({
      ...prev,
      fullName: [crmUser.firstName, crmUser.lastName].filter(Boolean).join(' ').trim() || prev.fullName,
      nickname: String(crmUser.nickname || prev.nickname || '').trim(),
      title: String(crmUser.title || prev.title || '').trim(),
      email: String(crmUser.email || prev.email || '').trim(),
      role: String(crmUser.role || prev.role || '').trim(),
      team: String(crmUser.teamName || prev.team || '').trim(),
      authType: 'Cognito',
      status: crmUser.isActive === false ? 'Inactive' : 'Active',
      photoDataUrl: String(crmUser.profilePhotoUrl || prev.photoDataUrl || '').trim(),
    }));
  }, [COGNITO_ENABLED, cognitoCrmUserState]);
  ...
  React.useEffect(() => {
    ...
    async function hydrateAccountsFromBackend() {
      try {
        ...
        if (!cancelled && Array.isArray(backendAccounts)) {
          setAccountList(sanitizeAccounts(backendAccounts));
        }
      } catch (error) {
...
function openOpportunityDetail(opportunityId) {
  ...
  if (COGNITO_ENABLED && !cognitoCrmUserState?.payload?.data?.mapped) {
    return (
      <div style={{ minHeight: '100vh', height: '100vh', overflowY: 'auto', overflowX: 'hidden', background: '#031E22', color: '#F3F7F7', display: 'block', padding: '20px', fontFamily: 'Inter, Arial, sans-serif' }}>
        <div style={{ width: '100%', maxWidth: '780px', margin: '0 auto', borderRadius: '24px', border: '1px solid #0E545D', background: 'linear-gradient(180deg, #052F35 0%, #04252A 100%)', boxShadow: '0 24px 60px rgba(0,0,0,0.32)', overflow: 'hidden' }}>
          <div style={{ padding: '16px 20px', borderBottom: '1px solid #0E545D', background: 'linear-gradient(90deg,#063F47 0%,#0A7983 42%,#21C8D3 72%,#239EE2 100%)', color: '#FFFFFF' }}>
            <div style={{ fontSize: '11px', fontWeight: 800, letterSpacing: '0.24em', textTransform: 'uppercase', opacity: 0.92 }}>
              NAES CRM Security
            </div>
          ...
          <div style={{ padding: '18px 20px 20px 20px', display: 'grid', gap: '14px' }}>
            <div style={{ borderRadius: '20px', border: '1px solid #0E545D', background: '#054E57', padding: '18px 18px 16px 18px' }}>
              <div style={{ fontSize: '11px', fontWeight: 800, letterSpacing: '0.20em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.62)' }}>
                Access mode
              </div>
              <div style={{ marginTop: '10px', fontSize: '24px', fontWeight: 800, color: '#FFFFFF' }}>
                Cognito authentication is enabled
              </div>
              <p style={{ margin: '10px 0 0 0', fontSize: '14px', lineHeight: 1.7, color: 'rgba(255,255,255,0.82)' }}>
                The CRM shell is intentionally blocked until the Cognito login flow is fully wired. This protects the application from falling back to the managed development shim when Cognito mode is active.
              </p>

              <div style={{ marginTop: '14px', display: 'grid', gap: '8px', padding: '12px 14px', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.12)', background: 'rgba(0,0,0,0.10)' }}>
                <div style={{ fontSize: '10px', fontWeight: 800, letterSpacing: '0.18em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.58)' }}>
                  Live auth state
                </div>
                <div style={{ fontSize: '13px', color: '#FFFFFF', wordBreak: 'break-word' }}>
                  exchange_status={cognitoCallbackState.exchangeStatus || 'pending'}
                </div>
                <div style={{ fontSize: '13px', color: '#FFFFFF', wordBreak: 'break-word' }}>
                  exchange_error={cognitoCallbackState.exchangeError || '—'}
                </div>
                <div style={{ fontSize: '13px', color: '#FFFFFF', wordBreak: 'break-word' }}>
                  crm_map_status={cognitoCrmUserState.status || 'pending'}
                </div>
                <div style={{ fontSize: '13px', color: '#FFFFFF', wordBreak: 'break-word' }}>
                  crm_map_error={cognitoCrmUserState.error || '—'}
                </div>
                <div style={{ fontSize: '13px', color: '#FFFFFF', wordBreak: 'break-word' }}>
                  mapped={String(Boolean(cognitoCrmUserState?.payload?.data?.mapped))}
                </div>
                <div style={{ fontSize: '13px', color: '#FFFFFF', wordBreak: 'break-word' }}>
                  crm_user_id={cognitoCrmUserState?.payload?.data?.crmUser?.id || '—'}
                </div>
                <div style={{ fontSize: '13px', color: '#FFFFFF', wordBreak: 'break-word' }}>
                  crm_email={cognitoCrmUserState?.payload?.data?.crmUser?.email || '—'}
                </div>
                <div style={{ fontSize: '13px', color: '#FFFFFF', wordBreak: 'break-word' }}>
                  auth_state_user={authState?.currentUser?.email || '—'}
                </div>
              </div>
            </div>

            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '12px' }}>
              <div style={{ borderRadius: '18px', border: '1px solid #0E545D', background: '#043941', padding: '16px' }}>
                <div style={{ fontSize: '10px', fontWeight: 800, letterSpacing: '0.18em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.58)' }}>
                  Auth mode
                </div>
                <div style={{ marginTop: '8px', fontSize: '15px', fontWeight: 700, color: '#FFFFFF' }}>
                  {AUTH_MODE}
                </div>
              </div>
              ...
              <div style={{ borderRadius: '18px', border: '1px solid #0E545D', background: '#043941', padding: '16px' }}>
                ...
                <div style={{ marginTop: '8px', fontSize: '15px', fontWeight: 700, color: '#FFFFFF', wordBreak: 'break-word' }}>
                  {API_BASE_URL}
                </div>
              </div>
            </div>
            ...
            <div style={{ display: 'grid', gap: '12px' }}>
              ...
              <div style={{ borderRadius: '18px', border: '1px solid #0E545D', background: '#043941', padding: '16px' }}>
                ...
                <div style={{ marginTop: '8px', fontSize: '15px', fontWeight: 700, color: '#FFFFFF', wordBreak: 'break-word' }}>
                  {COGNITO_APP_CLIENT_ID || 'Not configured'}
                </div>
              </div>
              ...
              <div style={{ borderRadius: '18px', border: '1px solid #0E545D', background: '#043941', padding: '16px' }}>
                ...
                <div style={{ marginTop: '8px', fontSize: '15px', fontWeight: 700, color: '#FFFFFF', wordBreak: 'break-word' }}>
                  {COGNITO_REDIRECT_SIGN_IN || 'Not configured'}
                </div>
              </div>
              ...
              <div style={{ borderRadius: '18px', border: '1px solid #0E545D', background: '#043941', padding: '16px' }}>
                ...
                <div style={{ marginTop: '8px', fontSize: '15px', fontWeight: 700, color: '#FFFFFF', wordBreak: 'break-word' }}>
                  {COGNITO_REDIRECT_SIGN_OUT || 'Not configured'}
                </div>
              </div>
            </div>

            {(cognitoCallbackState.code || cognitoCallbackState.error) ? (
              <div style={{ borderRadius: '18px', border: cognitoCallbackState.error || cognitoCallbackState.exchangeStatus === 'error' ? '1px solid rgba(177,18,38,0.45)' : '1px solid #0E545D', background: cognitoCallbackState.error || cognitoCallbackState.exchangeStatus === 'error' ? 'rgba(177,18,38,0.12)' : '#043941', padding: '16px', display: 'grid', gap: '10px' }}>
                <div style={{ fontSize: '10px', fontWeight: 800, letterSpacing: '0.18em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.58)' }}>
                  Cognito callback result
                </div>
                {cognitoCallbackState.code ? (
                  <>
                    <div style={{ fontSize: '15px', fontWeight: 700, color: '#FFFFFF' }}>
                      Authorization code returned successfully
                    </div>
                    <div style={{ fontSize: '13px', color: 'rgba(255,255,255,0.82)', wordBreak: 'break-all' }}>
                      code={cognitoCallbackState.code}
                    </div>
                    <div style={{ fontSize: '13px', color: 'rgba(255,255,255,0.82)', wordBreak: 'break-word' }}>
                      exchange_status={cognitoCallbackState.exchangeStatus || 'pending'}
                    </div>
                    {cognitoCallbackState.exchangeError ? (
                      <div style={{ fontSize: '13px', color: 'rgba(255,255,255,0.82)', wordBreak: 'break-word' }}>
                        exchange_error={cognitoCallbackState.exchangeError}
                      </div>
                    ) : null}
                    {cognitoCallbackState.exchangePayload?.data?.id_claims?.email ? (
                      <div style={{ fontSize: '13px', color: 'rgba(255,255,255,0.82)', wordBreak: 'break-word' }}>
                        id_claims.email={cognitoCallbackState.exchangePayload.data.id_claims.email}
                      </div>
                    ) : null}
                    {cognitoCallbackState.exchangePayload?.data?.id_claims?.sub ? (
                      <div style={{ fontSize: '13px', color: 'rgba(255,255,255,0.82)', wordBreak: 'break-word' }}>
                        id_claims.sub={cognitoCallbackState.exchangePayload.data.id_claims.sub}
                      </div>
                    ) : null}
                  </>
                ) : null}
                {cognitoCallbackState.error ? (
                  <>
                    <div style={{ fontSize: '15px', fontWeight: 700, color: '#FFFFFF' }}>
                      Cognito returned an error
                    </div>
                    <div style={{ fontSize: '13px', color: 'rgba(255,255,255,0.82)', wordBreak: 'break-word' }}>
                      error={cognitoCallbackState.error}
                    </div>
                    <div style={{ fontSize: '13px', color: 'rgba(255,255,255,0.82)', wordBreak: 'break-word' }}>
                      description={cognitoCallbackState.errorDescription || '—'}
                    </div>
                  </>
                ) : null}
              </div>
            ...
            {cognitoAuthenticatedIdentity ? (
              <div style={{ borderRadius: '18px', border: '1px solid #0E545D', background: '#054E57', padding: '16px', display: 'grid', gap: '10px' }}>
                <div style={{ fontSize: '10px', fontWeight: 800, letterSpacing: '0.18em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.58)' }}>
                ...
                <div style={{ fontSize: '13px', color: 'rgba(255,255,255,0.82)', wordBreak: 'break-word' }}>
                  email={cognitoAuthenticatedIdentity.email || '—'}
                </div>
                <div style={{ fontSize: '13px', color: 'rgba(255,255,255,0.82)', wordBreak: 'break-word' }}>
                  name={cognitoAuthenticatedIdentity.name || '—'}
                </div>
                <div style={{ fontSize: '13px', color: 'rgba(255,255,255,0.82)', wordBreak: 'break-word' }}>
                  sub={cognitoAuthenticatedIdentity.sub || '—'}
                </div>
                <div style={{ fontSize: '13px', color: 'rgba(255,255,255,0.82)', wordBreak: 'break-word' }}>
                  cognito_username={cognitoAuthenticatedIdentity.username || '—'}
                </div>
                <div style={{ fontSize: '13px', color: 'rgba(255,255,255,0.82)', wordBreak: 'break-word' }}>
                  crm_map_status={cognitoCrmUserState.status || 'pending'}
                </div>
                {cognitoCrmUserState.error ? (
                  <div style={{ fontSize: '13px', color: 'rgba(255,255,255,0.82)', wordBreak: 'break-word' }}>
                    crm_map_error={cognitoCrmUserState.error}
                  </div>
                ) : null}
                {cognitoCrmUserState?.payload?.data?.crmUser?.id ? (
                  <div style={{ fontSize: '13px', color: 'rgba(255,255,255,0.82)', wordBreak: 'break-word' }}>
                    crm_user_id={cognitoCrmUserState.payload.data.crmUser.id}
                  </div>
                ) : null}
                <div style={{ fontSize: '12px', color: 'rgba(255,255,255,0.62)' }}>
                  {cognitoCrmUserState?.payload?.data?.mapped
                    ? 'CRM user mapping succeeded. The shell is now authorized to open under the mapped CRM user.'
                    : 'CRM shell remains intentionally locked until Cognito identity is mapped to a CRM user record.'}
                </div>
              </div>
            ) : null}

            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '14px', flexWrap: 'wrap' }}>
              <div style={{ fontSize: '12px', color: 'rgba(255,255,255,0.58)', maxWidth: '520px' }}>
                Cognito mode is now configured enough to begin Hosted UI sign-in testing. The shell will remain blocked until callback handling is wired.
              </div>

              <button
                type="button"
                onClick={() => {
                  const nextUrl = buildCognitoAuthorizeUrl();
                  if (!nextUrl) {
                    window.alert('Cognito sign-in configuration is incomplete.');
                    return;
                  }
...
: safeActivePage === 'Settings'
              ? (
                <SettingsPage
                  ...
                  onPersistUserProfile={persistUserProfileToApi}
              ...
              ? (
                <UserAccountsPage
                  ...
                  onCreateUserAccount={createUserViaApiInApp}
                  onUpdateUserAccount={updateUserViaApiInApp}
                />
              )
              : (
