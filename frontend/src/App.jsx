    ...
    if (this.state.hasError) {
      return (
        <div style={{ border: '1px solid #fde68a', background: '#fffbeb', color: '#92400e', borderRadius: '14px', padding: '14px', fontSize: '13px', fontWeight: 800 }}>
          Signal analytics chart failed to render. The rest of Signal is still available.
        </div>
      );
    }
...
function SettingsPage({ userProfile = {}, userAccounts = [], onSaveUserProfile, onSaveUserAccounts }) {
  ...
  useEffect(() => {
    ...
    setImageError('');
  }, [userProfile?.id, userProfile?.nickname, userProfile?.photoDataUrl]);
    ...
    reader.onload = () => {
      ...
      img.onload = () => {
        try {
          ...
          if (compressed) {
            setDraftPhotoDataUrl(String(compressed));
          }
          setImageError('');
        } catch (error) {
          setDraftPhotoDataUrl(source);
  ...
  async function handleSave() {
    ...
    if (!profileId) {
      console.error('PROFILE_DIRECT_SAVE_MISSING_ID');
      setImageError('Profile could not be saved. Missing user id.');
      return;
    }

    try {
      console.log('PROFILE_DIRECT_SAVE_START', {
        profileId,
        nickname: nextProfile.nickname,
        photoLen: String(nextProfile.photoDataUrl || '').length
      });
      ...
      const response = await fetch(`/api/profile/${profileId}`, {
        ...
        body: JSON.stringify({
          nickname: String(nextProfile.nickname || '').trim() || null,
          profilePhotoUrl: String(nextProfile.photoDataUrl || '').trim() || null,
        }),
      });

      const json = await response.json();

      console.log('PROFILE_DIRECT_SAVE_RESULT', {
      ...
      if (!response.ok || !json?.ok) {
        throw new Error(json?.error || `Profile update failed: ${response.status}`);
      }

      const savedProfile = {
        ...nextProfile,
        photoDataUrl: String(json?.data?.profilePhotoUrl || nextProfile.photoDataUrl || ''),
        nickname: String(json?.data?.nickname || nextProfile.nickname || '')
      };

      window.localStorage.setItem('naes-crm-user-profile', JSON.stringify(savedProfile));
      setImageError('');
      await onSaveUserProfile(savedProfile);
    } catch (error) {
      console.error('Profile save failed', error);
      setImageError('Profile could not be saved. Please try again.');
    }
  }
      ...
      <section style={{ ...naesCardStyle(false), padding: '24px', display: 'grid', gap: '18px' }}>
        ...
        <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-start', flexWrap: 'wrap' }}>
          <button
            ...
            style={{
              borderRadius: '14px',
              border: `1px solid ${hasChanges ? '#0B6E4F' : naesTheme.border}`,
              background: hasChanges ? '#0B6E4F' : '#F3F5F4',
              color: hasChanges ? '#FFFFFF' : naesTheme.textSoft,
              padding: '10px 18px',
              fontSize: '14px',
              fontWeight: 800,
              cursor: hasChanges ? 'pointer' : 'not-allowed',
              boxShadow: hasChanges ? '0 6px 18px rgba(11,110,79,0.18)' : 'none'
            }}
          >
            Save New Photo
          </button>
...
function UserAccountsPage({
  ...
  function handleSave() {
    if (isCreatingNew) {
      const fullName = String(draftUser.fullName || '').trim();
      const parts = fullName.split(/\s+/).filter(Boolean);
      const email = String(draftUser.email || '').trim();

      if (parts.length < 2) {
        window.alert('Please enter both first and last name in the Full Name field.');
        return;
      }

      if (!email) {
        window.alert('Please enter a work email before creating the user.');
        return;
      }
      ...
      Promise.resolve(onCreateUserAccount ? onCreateUserAccount(nextUser) : nextUser)
        .then((createdUser) => {
          const nextAccounts = [createdUser, ...safeAccounts];
          onSaveUserAccounts(nextAccounts);
          setIsCreatingNew(false);
          setSelectedUserId(createdUser.id);
          setDraftUser(buildUserAccountDraft(createdUser));
        })
        .catch((error) => {
          console.warn('Could not create user in backend', error);
        });
      return;
    }
    ...
    Promise.resolve(onUpdateUserAccount ? onUpdateUserAccount(selectedRecord.id, draftUser) : draftUser)
      .then((updatedUser) => {
        ...
        if (draftUser.linkedProfile) {
          const nextProfile = {
            ...
            team: draftUser.team,
            isActive: draftUser.status !== 'Inactive',
      ...
      .catch((error) => {
        console.warn('Could not update user in backend', error);
      });
  }
    ...
    async function handleDelete() {
      ...
      if (!confirm('Delete this user from the admin directory?')) return;

      const response = await fetch(`/api/admin-users/${selectedRecord.id}`, {
        method: 'DELETE',
        credentials: 'include'
      });
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '340px minmax(0, 1fr)', gap: '24px', alignItems: 'start' }}>
        ...
        <section style={{ ...naesCardStyle(false), padding: '24px', display: 'grid', gap: '18px', maxWidth: '576px', width: '100%' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '88px minmax(0, 1fr)', gap: '18px', alignItems: 'center' }}>
            ...
            <div>
              {smallLabel(isCreatingNew ? 'New User' : 'Selected User')}
          ...
          <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
            ...
            >
              {isCreatingNew ? 'Discard New User' : 'Delete User'}
            </button>
          </div>
...
function renderExecutiveDashboard({ accounts = [], contacts = [], opportunities = [], onOpenOpportunity } = {}) {
  ...
  function ownerFor(item) {
    const nestedOwnerName = [item?.owner?.firstName, item?.owner?.lastName].filter(Boolean).join(' ').trim();
    const directOwner = typeof item?.owner === 'string' ? item.owner : '';
    return String(item?.owner_full_name ?? nestedOwnerName ?? directOwner ?? 'Unassigned').trim() || 'Unassigned';
  }
  ...
  const currentUserName = String(userProfile?.fullName || '').trim();
  const currentUserId = String(userProfile?.id || '').trim();

  const myOpportunities = opportunities.filter((item) => {
    const itemOwnerId = String(item?.ownerUserId || item?._backend?.ownerUserId || '').trim();
    const itemOwnerName = ownerFor(item);
    return (
      (currentUserId && itemOwnerId && itemOwnerId === currentUserId) ||
      (currentUserName && itemOwnerName === currentUserName)
    );
  });
...
function formatServiceLine(value) {
  ...
  if (raw === 'Renewables O&M') return 'Renewables';
  if (raw === 'Other O&M / Custom Services') return 'Other O&M';

  return raw || '—';
}
function serviceLineFor(item) {
                    {formatServiceLine(item?.service_line || item?.serviceLine)}
...
function BusinessReviewsPage({ opportunities = [], accounts = [], contacts = [], tasks = [], activities = [] }) {
  ...
  function serviceLineFor(item) {
  const raw = String(item?.service_line ?? item?.serviceLine ?? 'Unassigned').trim();
  ...
  return (
    <div style={{
      ...
      <section style={{
        ...
        minHeight: '430px',
        borderRadius: 22,
        border: '1px solid rgba(11, 103, 113, 0.18)',
        ...
        <div style={{
          position: 'absolute',
          inset: 0,
          background: 'radial-gradient(circle at 18% 92%, rgba(11, 103, 113, 0.12), transparent 34%), radial-gradient(circle at 84% 88%, rgba(26, 166, 191, 0.14), transparent 36%)',
          pointerEvents: 'none'
        }} />
        ...
        <div style={{ position: 'relative', zIndex: 1, maxWidth: 660 }}>
          <div style={{
            ...
            margin: '0 auto 22px',
            borderRadius: '50%',
            background: 'linear-gradient(135deg, rgba(11,103,113,0.12), rgba(127,179,213,0.22))',
            display: 'grid',
            placeItems: 'center',
            fontSize: 52,
            boxShadow: 'inset 0 0 0 1px rgba(11,103,113,0.14)'
          }}>
            📊
          </div>
          ...
          <div style={{
            ...
            height: 2,
            margin: '24px auto',
            background: 'linear-gradient(90deg, transparent, #0B6771, #20B7C7, #0B6771, transparent)'
          }} />
          ...
          <p style={{
            ...
            We are building a polished reporting workspace for customer-ready summaries, portfolio insights, executive views, and CRM-connected report generation.
          </p>
          ...
          <div style={{
            ...
            borderRadius: 999,
            padding: '12px 18px',
            background: 'linear-gradient(135deg, #043F46, #0B6771)',
            color: '#fff',
            fontSize: 14,
            fontWeight: 900,
            boxShadow: '0 12px 26px rgba(11,103,113,0.28)'
          }}>
            <span>🔔</span>
            Stay Tuned
          </div>

          <div style={{ marginTop: 14, fontSize: 13, color: '#64748b', fontWeight: 700 }}>
...
<section style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0, 1fr))', gap: '12px' }}>
  {[
    { label: 'Open', value: openTasks.length, color: '#2563EB' },
    { label: 'Overdue', value: tasks.filter(t => t.dueDate && new Date(t.dueDate) < new Date() && t.status !== 'Completed').length, color: '#DC2626' },
    { label: 'Due Soon', value: tasks.filter(t => {
    ...
    { label: 'Completed', value: completedTasks.length, color: '#16A34A' }
  ].map(card => (
              ...
              {['Open', 'Completed'].map((tab) => (
                <button
                  ...
                  style={{
                    ...
                    background: taskTab === tab ? '#0B6771' : '#F7FBF8',
                    color: taskTab === tab ? '#fff' : '#18342a',
                    fontWeight: 700,
                    cursor: 'pointer'
                  }}
                >
  ...
  <button
    ...
    style={{
      ...
      borderRadius: 6,
      border: '1px solid #DCE7DD',
      background: task.status === 'Completed' ? '#EEF7F0' : '#F7FBF8',
      cursor: 'pointer'
    }}
  >
...
<section style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0, 1fr))', gap: '12px' }}>
  {[
    { label: 'Total', value: totalActivities, color: '#2563EB' },
    { label: 'This Week', value: activities.filter(a => {
        if (!a.activityDateTime) return false;
        const d = new Date(a.activityDateTime);
        const now = new Date();
        const week = new Date();
        week.setDate(now.getDate() - 7);
        return d >= week;
      }).length, color: '#0B6771' },
    { label: 'Meetings', value: meetings, color: '#7C3AED' },
    { label: 'Needs Follow-Up', value: activities.filter(a => !a.nextStep).length, color: '#DC2626' }
  ].map(card => (
    <div key={card.label} style={{
      ...naesCardStyle(false),
      padding: '10px 12px',
...
function ActivitiesOverviewPage({ activities = [], accounts = [], contacts = [], opportunities = [], onStartNewActivity, onOpenActivity }) {
  function groupActivitiesByTime(list = []) {
    const now = new Date();
    ...
    <div style={{ maxWidth: '1280px', margin: '0 auto', display: 'grid', gap: '24px' }}>
        ...
        <section style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0, 1fr))', gap: '12px' }}>
          {[
            ...
            { label: 'Meetings', value: meetings, color: '#7C3AED' },
            { label: 'Needs Follow-Up', value: activities.filter(a => !String(a.nextStep || '').trim()).length, color: '#DC2626' }
          ].map(card => (
            <div key={card.label} style={{
              ...naesCardStyle(false),
              padding: '10px 12px',
              ...
              <div style={{ fontSize: 20, fontWeight: 800, color: naesTheme.text }}>
                {card.value}
              </div>
            </div>
          ))}
    ...
    return (
      <div style={{ marginBottom: 16 }}>
        ...
        {items.map((activity) => (
          ...
          >
            <div style={{ marginRight: 10 }}>{iconForActivity(activity.activityType)}</div>

            <div style={{ flex: 1 }}>
              <div style={{ fontWeight: 500 }}>{activity.subject || 'Activity'}</div>
              <div style={{ fontSize: 11, color: '#6B7280' }}>
                {activity.owner || '—'} • {activity.activityDateTime || '—'}
              </div>
            </div>
          </div>
        ))}
...
function buildOpportunityFormFromRecord(opportunity = {}) {
  return {
    ...
    serviceLine: String(opportunity.service_line || opportunity.serviceLine || 'Renewables'),
      stratoSqftTotal: String(opportunity.estimated_square_footage || ''),
      ...
      <section style={shellCard}>
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: '16px', flexWrap: 'wrap', alignItems: 'flex-start' }}>
          <div>
            ...
            <h2 style={{ margin: '8px 0 0 0', fontSize: '24px', fontWeight: 600 }}>Add New Deal</h2>
      ...
      <section style={shellCard}>
        ...
        <div style={{ ...formColumnStyle, marginTop: '16px', display: 'grid', gap: '16px' }}>
          ...
          <div>
            ...
            <select style={fieldStyle} value={newOpportunityForm.owner || ''} onChange={(e) => onChangeNewOpportunityField('owner', e.target.value)}>
      <option value="">Select Owner</option>
      {(userOptions || []).map((u) => (
                <option key={u.id} value={String(u.fullName || '').trim()}>
...
function NewAccountPage({ newAccountForm = {}, accountOptions = [], users = [], isEditMode = false, onChangeNewAccountField, onSaveNewAccount, onCancelNewAccount }) {
      ...
      <section style={sectionStyle}>
        ...
        <div style={stackedGrid}>
          ...
          <div>
            ...
            <select
              value={newAccountForm.ownerUserId ?? ''}
              onChange={(e) => onChangeNewAccountField('ownerUserId', e.target.value)}
              style={inputStyle}
            >
              <option value="">Select Owner</option>
              {(users || []).map((u) => (
...
function OpportunityDetailPage({ onBackToOverview, opportunity, onSaveOpportunity, users = [], setActiveOpportunity }) {
  const detailForecastCategories = ['Pipeline', 'Best Case', 'Commit', 'Closed Won', 'Closed Lost'];
...
React.useEffect(() => {
  ...
  const opportunityPills = useMemo(() => buildOpportunityPills({
    ...
    owner_full_name: String(opportunity?.owner_full_name || opportunity?.ownerName || opportunity?.owner || '').trim() || '—',
      ...
      <div>
        ...
        <button
          onClick={() => {
            ...
            if (setActiveOpportunity) setActiveOpportunity(opportunity);
            if (!id) return;
            pushPath(`/opportunities/${id}`);
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: '20px', flexWrap: 'wrap', alignItems: 'flex-start' }}>
          <div>
            ...
            <div style={{ marginTop: '14px', maxWidth: '360px' }}>
              <div style={{ fontSize: '10px', fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', color: '#64748b', marginBottom: '6px' }}>
                NAES Opportunity Owner
              </div>
              <select
                style={{ width: '100%', borderRadius: '12px', border: '1px solid #D8E2D9', background: '#fff', padding: '10px 12px', fontSize: '14px', color: '#0f172a' }}
                value={String(opportunity?._backend?.ownerUserId || opportunity?.ownerUserId || '')}
                onChange={(e) => {
                  const nextOwnerId = String(e.target.value || '').trim();
                  if (!opportunity?.id || !onSaveOpportunity) return;
                  onSaveOpportunity(opportunity.id, {
        ...
        <div style={{ display: 'grid', gap: '24px' }}>
          ...
          <section style={shellCard}>
            ...
            <div style={{ marginTop: '18px', display: 'grid', gap: '14px', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))' }}>
              <div style={{ borderRadius: '16px', border: '1px solid #E5ECE5', background: '#FBFCFB', padding: '14px' }}>
                <div style={{ fontSize: '10px', fontWeight: 700, letterSpacing: '0.16em', textTransform: 'uppercase', color: '#64748b' }}>Stage</div>
                <select
                  style={{ marginTop: '6px', width: '100%', borderRadius: '10px', border: '1px solid #D8E2D9', background: '#fff', padding: '10px 12px', fontSize: '14px', fontWeight: 600, color: '#111827' }}
                  value={String(opportunity?.stage || '').trim()}
                  onChange={(e) => {
                    const nextStage = String(e.target.value || '').trim();
                    if (!opportunity?.id || !onSaveOpportunity) return;
                    onSaveOpportunity(opportunity.id, { stage: nextStage });
                  }}
                >
                  {workflowStages.map((stage) => (
                    <option key={stage} value={stage}>{stage}</option>
              ...
              <div style={{ borderRadius: '16px', border: '1px solid #E5ECE5', background: '#FBFCFB', padding: '14px' }}>
                <div style={{ fontSize: '10px', fontWeight: 700, letterSpacing: '0.16em', textTransform: 'uppercase', color: '#64748b' }}>Forecast Category</div>
                <select
                  style={{ marginTop: '6px', width: '100%', borderRadius: '10px', border: '1px solid #D8E2D9', background: '#fff', padding: '10px 12px', fontSize: '14px', fontWeight: 600, color: '#111827' }}
                  value={String(opportunity?.forecast_category || '').trim()}
                  onChange={(e) => {
                    const nextForecast = String(e.target.value || '').trim();
                    if (!opportunity?.id || !onSaveOpportunity) return;
                    onSaveOpportunity(opportunity.id, { forecast_category: nextForecast });
                  }}
                >
                  {detailForecastCategories.map((category) => (
              ...
              <div style={{ borderRadius: '16px', border: '1px solid #E5ECE5', background: '#FBFCFB', padding: '14px' }}>
                <div style={{ fontSize: '10px', fontWeight: 700, letterSpacing: '0.16em', textTransform: 'uppercase', color: '#64748b' }}>Expected Close</div>
                <input
                  type="date"
                  style={{ marginTop: '6px', width: '100%', borderRadius: '10px', border: '1px solid #D8E2D9', background: '#fff', padding: '10px 12px', fontSize: '14px', fontWeight: 600, color: '#111827' }}
                  value={String(opportunity?.expected_close_date || '').trim().slice(0, 10)}
                  onChange={(e) => {
                    const nextClose = String(e.target.value || '').trim();
                    if (!opportunity?.id || !onSaveOpportunity) return;
                    onSaveOpportunity(opportunity.id, { expected_close_date: nextClose });
                  }}
                />
              </div>
            </div>
            ...
            <div style={{ marginTop: '16px', display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
              {workflowStages.map((stage) => {
                const active = String(opportunity?.stage || '').trim() === String(stage).trim();
                return (
                  <span
                    key={stage}
                    style={{
                      ...
                      fontSize: '12px',
                      fontWeight: 500,
                      background: active ? '#0B6771' : '#F2F5F2',
...
function parseOpportunityRoute(pathname) {
  ...
  if (pathname === '/new-deal-v2') {
    return { isOpportunities: true, detailId: null, isNewDealV2: true };
  }
  if (pathname === '/opportunities-v2-dashboard') {
    return { isOpportunities: true, detailId: null, isOpportunitiesV2: true };
  }
  ...
  const v2match = pathname.match(/^\/opportunities-v2\/([^/]+)$/);
  if (v2match) {
    return { isOpportunities: true, detailId: v2match[1], isV2: true };
  }
...
export default function App() {
  ...
  const [apolloSearchResults, setApolloSearchResults] = useState([]);
  const [apolloSearchStatus, setApolloSearchStatus] = useState('Preview Mode');
  const [apolloSearchError, setApolloSearchError] = useState('');
  const [signalSelectedProspect, setSignalSelectedProspect] = useState(null);
  const [signalQueue, setSignalQueue] = useState([]);

  // === SIGNAL CAMPAIGNS STATE ===
  const [signalCampaigns, setSignalCampaigns] = useState([]);
  const [selectedCampaignId, setSelectedCampaignId] = useState('');
  const [newCampaignName, setNewCampaignName] = useState('');
  const [sendPreviewPayloads, setSendPreviewPayloads] = useState({});
  const [sendResultSummaries, setSendResultSummaries] = useState({});
  const [signalEvents, setSignalEvents] = useState([]);
  const [signalSuppressions, setSignalSuppressions] = useState([]);
  const [signalReadiness, setSignalReadiness] = useState({ mode: 'safe-mode', live_ready: false, checks: [] });
  const [signalSendMode, setSignalSendMode] = useState('safe');
  ...
  const addManualContactToCampaign = async (campaignId) => {
    if (!campaignId || !manualContact.email) return;

    try {
      await backendFetch(`/api/signal/campaigns/${campaignId}/contacts`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ contacts: [manualContact] })
      });
  ...
  const [suppressionEventType, setSuppressionEventType] = useState('bounced');

  // === TEMPLATE ENGINE ===
  const [campaignSubject, setCampaignSubject] = useState('Quick question for {{company}}');
...
`
  ...
  const simulateSuppressionEvent = async () => {
    if (!suppressionEmail.trim()) return;

    try {
      await backendFetch('/api/signal/campaigns/events/log', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          event_type: suppressionEventType,
          email: suppressionEmail,
          contact_id: suppressionEmail,
          reason: suppressionReason || `Simulated ${suppressionEventType} event`,
          source: 'signal-ui-simulator',
          payload: {
            email: suppressionEmail,
            reason: suppressionReason || `Simulated ${suppressionEventType} event`
          }
      ...
      await loadSignalSuppressions();
    } catch (err) {
      console.error('Failed to simulate suppression event', err);
    }
  };

  const removeSignalSuppression = async (email) => {
    if (!email) return;

    try {
      await backendFetch(`/api/signal/campaigns/suppressions/${encodeURIComponent(email)}`, {
        method: 'DELETE'
      });
      await loadSignalSuppressions();
    } catch (err) {
  ...
  const createSignalSuppression = async () => {
    if (!suppressionEmail.trim()) return;

    try {
      await backendFetch('/api/signal/campaigns/suppressions', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: suppressionEmail, type: 'manual', reason: suppressionReason || 'Manual suppression from Signal UI' })
      });
  ...
  const loadSignalReadiness = async () => {
    try {
      const res = await backendFetch('/api/signal/campaigns/readiness');
      const data = await res.json();
      const nextReadiness = data || { mode: 'safe-mode', live_ready: false, checks: [] };
      setSignalReadiness(nextReadiness);
      if (!nextReadiness.live_ready) setSignalSendMode('safe');
    } catch (err) {
      console.warn('Failed to load signal readiness', err);
    }
  };

  const loadSignalSuppressions = async () => {
    try {
      const res = await backendFetch('/api/signal/campaigns/suppressions');
      const data = await res.json();
      setSignalSuppressions(data.suppressions || []);
    } catch (err) {
      console.warn('Failed to load signal suppressions', err);
    }
  };

  const loadSignalEvents = async () => {
    try {
      const res = await backendFetch('/api/signal/campaigns/events');
      const data = await res.json();
      setSignalEvents(data.events || []);
    } catch (err) {
      console.warn('Failed to load signal events', err);
    }
  };

  const logSignalEvent = async (event = {}) => {
    try {
      await backendFetch('/api/signal/campaigns/events/log', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      console.warn('Failed to log signal event', err);
    }
  };

  const runSignalSafeSend = async (campaignId) => {
    if (!campaignId) return;

    try {
      const res = await backendFetch(`/api/signal/campaigns/${campaignId}/send`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ mode: signalSendMode })
      });

      const data = await res.json();

      setSendPreviewPayloads((prev) => ({
        ...prev,
        [campaignId]: data.send_results || []
      }));

      setSendResultSummaries((prev) => ({
        ...prev,
        [campaignId]: {
          mode: data.mode || signalSendMode,
      ...
      console.error('Safe send failed', err);
    }
  };

  const runSignalSimulatedSend = async (campaignId) => {
    if (!campaignId) return;

    try {
      const res = await backendFetch(`/api/signal/campaigns/${campaignId}/send-preview`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' }
      });
      ...
      await Promise.all(preview.map((payload) => logSignalEvent({
        event_type: 'sent_simulated',
        campaign_id: campaignId,
        contact_id: payload.to || null,
  ...
  const runSignalSendPreview = async (campaignId) => {
    if (!campaignId) return;
  ...
  const renderTemplate = (template, contact = {}) => {
    return template
      ...
      .replace(/{{service_fit}}/g, contact.service_fit || '')
      .replace(/{{signal_insight}}/g, contact.signal_insight || '');
  };



  const loadSignalCampaigns = async () => {
    try {
      const res = await backendFetch('/api/signal/campaigns');
      const data = await res.json();
      setSignalCampaigns(data.campaigns || []);
    } catch (err) {
      console.warn('Failed to load campaigns', err);
    }
  };

  const createSignalCampaign = async () => {
  ...
  const addQueueToCampaign = async () => {
    if (!selectedCampaignId || signalQueue.length === 0) return;

    try {
      await backendFetch(`/api/signal/campaigns/${selectedCampaignId}/contacts`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ contacts: signalQueue })
      });
...
const clearSignalQueue = () => {
  ...
  const apolloPreviewRecords = [
    ...
    },
    {
      ...
      buyingSignal: 'Large industrial rooftop footprint with multi-site inspection and asset condition opportunity.',
      estimatedRevenue: '$720K',
      sourceStatus: 'Staged'
    },
    {
      company: 'Sierra Cold Chain Holdings',
      industry: 'Cold Storage',
      hq: 'Reno, NV',
      employees: '251-500',
      revenue: '$100M-$250M',
      ...
      contacts: 9,
      confidence: 'Medium',
      buyingSignal: 'High-energy facilities, thermal risk exposure, and potential O&M service alignment.',
      estimatedRevenue: '$310K',
      sourceStatus: 'Staged'
    }
  ];

  const activeApolloRecords = apolloSearchResults.length ? apolloSearchResults : apolloPreviewRecords;

  const runApolloPreviewSearch = async () => {
    setApolloSearchStatus('Searching...');
    setApolloSearchError('');

    try {
      const response = await backendFetch('/api/signal/apollo/search', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ query: apolloSearchTerm || 'signal preview search' })
      });

      if (!response.ok) {
        throw new Error(`Apollo preview failed with ${response.status}`);
      }

      const payload = await response.json();
      const results = Array.isArray(payload?.results) ? payload.results : [];

      setApolloSearchResults(results);
      setApolloSearchStatus(payload?.mode === 'preview' ? 'Backend Preview Connected' : 'Live Results');
    } catch (error) {
      console.error('Apollo preview search failed', error);
      setApolloSearchError(error.message || 'Apollo preview search failed');
      setApolloSearchStatus('Search Failed');
    }
  };
...
const signalTopScoreRows = scoredSignalProspects.slice(0, 6).map((prospect) => ({
  ...
  const signalEventCounts = signalEvents.reduce((acc, event) => {
    const key = event.event_type || 'unknown';
    acc[key] = (acc[key] || 0) + 1;
    return acc;
  }, {});

  const signalCampaignContactCount = signalCampaigns.reduce((sum, campaign) => sum + ((campaign.contacts || []).length), 0);

  const signalFunnelRows = [
    { stage: 'Contacts', value: signalCampaignContactCount },
    { stage: 'Previewed', value: signalEventCounts.preview_generated || 0 },
    { stage: 'Sent', value: signalEventCounts.sent_simulated || 0 },
    { stage: 'Opened', value: signalEventCounts.opened || 0 },
    { stage: 'Clicked', value: signalEventCounts.clicked || 0 },
    { stage: 'Replied', value: signalEventCounts.replied || 0 },
    { stage: 'Converted', value: signalEventCounts.converted || 0 }
  ];


  const signalEventTrendRows = Object.values(
    signalEvents.reduce((acc, event) => {
      const day = String(event.created_at || '').slice(0, 10) || 'Unknown';
      if (!acc[day]) {
        acc[day] = { day, previews: 0, sends: 0, opens: 0, clicks: 0, suppressions: 0 };
      }
      if (event.event_type === 'preview_generated') acc[day].previews++;
      if (event.event_type === 'sent_simulated' || event.event_type === 'sent') acc[day].sends++;
      if (event.event_type === 'opened') acc[day].opens++;
      if (event.event_type === 'clicked') acc[day].clicks++;
      if (['bounced','complained','unsubscribed'].includes(event.event_type)) acc[day].suppressions++;
      return acc;
    }, {})
  ).sort((a,b)=>String(a.day).localeCompare(String(b.day)));

  const signalEventRows = [
    { event: 'Preview', count: signalEventCounts.preview_generated || 0 },
    { event: 'Simulated Send', count: signalEventCounts.sent_simulated || 0 },
    { event: 'Opened', count: signalEventCounts.opened || 0 },
    { event: 'Clicked', count: signalEventCounts.clicked || 0 },
    { event: 'Replied', count: signalEventCounts.replied || 0 },
    { event: 'Bounced', count: signalEventCounts.bounced || 0 },
    { event: 'Converted', count: signalEventCounts.converted || 0 }
  ];
...
const signalExecutiveChartCard = {
  ...
  const seedSignalAccountIntake = (prospect = {}) => {
    const serviceFit = Array.isArray(prospect.serviceFit)
      ? prospect.serviceFit.filter(Boolean).join(', ')
      : String(prospect.serviceFit || 'Both').trim();

    const accountSeed = {
      name: prospect.name || prospect.company || '',
      industry: prospect.industry || prospect.businessType || 'Commercial / Industrial',
      businessType: prospect.industry || prospect.businessType || 'Commercial / Industrial',
      serviceLineFit: serviceFit || 'Both',
      interestedServices: serviceFit || 'Both',
      region: prospect.region || prospect.hq || '',
      generalFootprintRegion: prospect.region || prospect.hq || '',
      state: prospect.state || '',
      siteCount: prospect.sites ? String(prospect.sites) : '',
      estimatedBuildingCount: prospect.sites ? String(prospect.sites) : '',
      estimatedMw: prospect.mw ? String(prospect.mw) : '',
      totalMw: prospect.mw ? String(prospect.mw) : '',
      estimatedSqft: prospect.sqft ? String(prospect.sqft) : '',
      estimatedSquareFootage: prospect.sqft ? String(prospect.sqft) : '',
      notes: [
        'Source: Apollo / Signal',
        `Signal Score: ${prospect.score || 'n/a'}`,
        `Signal Priority: ${prospect.priority || 'n/a'}`,
        `Signal Insight: ${prospect.signalInsight || prospect.buyingSignal || 'n/a'}`,
        `Employees: ${prospect.employees || 'n/a'}`,
        `Estimated Revenue: ${prospect.estimatedRevenue || 'n/a'}`,
        `Apollo Contacts: ${prospect.contacts || 'n/a'}`,
        `Confidence: ${prospect.confidence || 'n/a'}`,
        `Market Segment: ${prospect.marketSegment || prospect.industry || 'Unknown'}`
      ].join('\n')
    };
    ...
    setSignalAccountSeed(accountSeed);
  ...
  const seedSignalContactIntake = (prospect = {}) => {
    const contactSeed = {
      ...
      accountId: '',
      accountName: prospect.name || prospect.company || '',
      email: '',
      mobilePhone: '',
      officePhone: '',
      website: '',
      ...
      state: prospect.region || prospect.hq || '',
      decisionMaker: false,
      champion: false,
      primaryContact: false,
      notes: [
        'Source: Apollo / Signal',
        `Target Account: ${prospect.name || prospect.company || 'Unknown'}`,
        `Industry: ${prospect.industry || 'Unknown'}`,
        `Region: ${prospect.region || prospect.hq || 'Unknown'}`,
        `Service Fit: ${Array.isArray(prospect.serviceFit) ? prospect.serviceFit.join(', ') : (prospect.serviceFit || 'Unknown')}`,
        `Buying Signal: ${prospect.signalInsight || prospect.buyingSignal || 'n/a'}`,
        `Employees: ${prospect.employees || 'n/a'}`,
        `Estimated Revenue: ${prospect.estimatedRevenue || 'n/a'}`,
        `Apollo Contacts: ${prospect.contacts || 'n/a'}`,
        `Confidence: ${prospect.confidence || 'n/a'}`
  ...
  const [signalAccountSeed, setSignalAccountSeed] = useState(null);
  ...
  function mapBackendAccountToFrontend(record = {}) {
    ...
    return {
      ...
      businessType: String(record?.accountType || record?.businessType || record?.industry || '').trim(),
      website: String(record?.website || '').trim(),
      linkedin: String(record?.linkedin || '').trim(),
      ...
      generalSiteType: String(record?.generalSiteType || '').trim(),
        strategicAccount: String(record?.strategicAccount || '').trim(),
        relationshipStatus: String(record?.relationshipStatus || '').trim(),
        decisionComplexity: String(record?.decisionComplexity || '').trim(),
        procurementType: String(record?.procurementType || '').trim(),
        customerDifficulty: String(record?.customerDifficulty || '').trim(),
        geographicSpread: String(record?.geographicSpread || '').trim(),
        accountStatus: String(record?.accountStatus || '').trim(),
      ...
      owner: String(record?.primaryAccountOwner || '').trim(),
      arr: 0,
      annualRevenue: 0,
      ctsPercent: 0,
      marginPercent: 0,
  ...
  function buildAccountApiPayload(form = {}) {
    return {
      ...
      accountType: String(form.industry || form.businessType || '').trim() || null,
      ...
      ownerUserId: String(form.ownerUserId || '').trim() || null,
        primaryAccountOwner: String(form.primaryAccountOwner || form.owner || '').trim() || null,
        accountStatus: String(form.accountStatus || form.status || '').trim() || null,
      notes: String(form.notes || '').trim() || null,

      legalBusinessName: String(form.legalBusinessName || '').trim() || null,
      linkedin: String(form.linkedin || '').trim() || null,
      interestedServices: String(form.serviceLineFit || form.interestedServices || '').trim() || null,
      totalMw: String(form.totalMw || '').trim() || null,
      portfolioType: String(form.portfolioType || '').trim() || null,
      generalFootprintRegion: String(form.region || form.generalFootprintRegion || '').trim() || null,
      ...
      generalSiteType: String(form.generalSiteType || '').trim() || null,
        strategicAccount: String(form.strategicAccount || '').trim() || null,
        relationshipStatus: String(form.relationshipStatus || '').trim() || null,
        decisionComplexity: String(form.decisionComplexity || '').trim() || null,
        procurementType: String(form.procurementType || '').trim() || null,
        customerDifficulty: String(form.customerDifficulty || '').trim() || null,
        geographicSpread: String(form.geographicSpread || '').trim() || null,
  ...
  async function deleteAccountViaApi(accountId) {
...
async function updateAccountViaApi(accountId, form = {}) {
  ...
  function backendUserRoleToFrontend(role = '') {
    const value = String(role || '').trim();
    if (value === 'ADMIN') return 'Admin';
    if (value === 'EXECUTIVE') return 'Executive';
    if (value === 'SALES_MANAGER') return 'Manager';
    if (value === 'SALES_ASSOCIATE') return 'Associate';
    return 'Associate';
  }

  function frontendUserRoleToBackend(role = '') {
    const value = String(role || '').trim().toLowerCase();
    if (value === 'admin') return 'ADMIN';
    if (value === 'executive') return 'EXECUTIVE';
    if (value === 'manager' || value === 'sales manager') return 'SALES_MANAGER';
    if (value === 'associate' || value === 'sales associate') return 'SALES_ASSOCIATE';
    return 'SALES_ASSOCIATE';
  }

  function mapBackendUserToFrontend(record = {}) {
    const fullName = [record?.firstName, record?.lastName].filter(Boolean).join(' ').trim();
    const frontendRole = backendUserRoleToFrontend(record?.role);
    const roleProfile = getRoleProfile(frontendRole);
    return buildUserAccountDraft({
      id: String(record?.id || ''),
      linkedProfile: false,
      fullName,
      nickname: String(record?.nickname || ''),
      ...
      role: frontendRole,
      department: '',
      team: String(record?.teamName || '').trim(),
      status: record?.isActive === false ? 'Inactive' : 'Active',
      permissionLevel: roleProfile.permissionLevel,
      businessUnitAccess: roleProfile.businessUnitAccess,
      serviceLineAccess: roleProfile.serviceLineAccess,
      territoryScope: roleProfile.territoryScope,
      authType: 'Managed Identity',
      passwordNote: 'Password is managed by administrator or identity provider.',
      lastLogin: '',
      photoDataUrl: String(record?.profilePhotoUrl || '')
    });
  }

  function buildUserApiPayload(record = {}) {
    const fullName = String(record?.fullName || '').trim();
    const parts = fullName.split(/\s+/).filter(Boolean);
    const firstName = String(parts[0] || '').trim();
    const lastName = String(parts.slice(1).join(' ') || '').trim();

    return {
      email: String(record?.email || '').trim(),
      firstName,
      lastName,
      nickname: String(record?.nickname || '').trim() || null,
      title: String(record?.title || '').trim() || null,
      role: frontendUserRoleToBackend(record?.role),
      teamName: String(record?.team || '').trim() || null,
      profilePhotoUrl: String(record?.photoDataUrl || '').trim() || null,
      isActive: String(record?.status || 'Active').trim().toLowerCase() !== 'inactive'
    };
  }
  ...
  async function fetchUsersFromApi() {
    ...
    const json = await response.json();
    const rows = Array.isArray(json?.data) ? json.data : [];
    return rows.map(mapBackendUserToFrontend);
  }

  async function createUserViaApi(record = {}) {
    const response = await backendFetch('/api/admin-users', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(buildUserApiPayload(record)),
    });
    ...
    const json = await response.json();
    return mapBackendUserToFrontend(json?.data || {});
  }

  async function updateUserViaApi(userId, record = {}) {
  ...
  async function saveUserProfileViaApi(profile = {}) {
    const profileId = String(profile?.id || userProfile?.id || '').trim();
    ...
    const response = await backendFetch(`/api/profile/${profileId}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        nickname: String(profile?.nickname || '').trim() || null,
        profilePhotoUrl: String(profile?.photoDataUrl || '').trim() || null,
      }),
    });
    ...
    const json = await response.json();
    const saved = mapBackendUserToFrontend(json?.data || {});
  ...
  function mapBackendOpportunityToFrontend(record = {}) {
    ...
    return {
      id: String(record?.id || ''),
      name: String(record?.name || 'New Deal').trim(),
      account: '',
      ...
      owner_team_name: String(record?.owner?.teamName || '').trim(),
      ...
      total_mwdc: null,
      total_mwac: null,
      estimated_square_footage: Number(record?.estimatedSquareFootage || record?.estimated_square_footage || 0) || null,
      ...
      account_name: String(record?.account?.name || '').trim(),
      last_activity_date: '',
  ...
  function buildOpportunityApiPayload(form = {}, fallbackRecord = {}) {
    return {
      ...
      name: String(form.name || fallbackRecord.name || 'New Deal').trim(),
      ...
      ownerUserId: String(form.ownerUserId || fallbackRecord.ownerUserId || fallbackRecord?._backend?.ownerUserId || '').trim() || null,
    };
  }


  function buildOpportunityPatchPayload(patch = {}) {
    const payload = {};
    const has = (key) => Object.prototype.hasOwnProperty.call(patch, key);
    const clean = (value) => String(value ?? '').trim();

    if (has('accountId')) payload.accountId = clean(patch.accountId) || null;
    if (has('primaryContactId')) payload.primaryContactId = clean(patch.primaryContactId) || null;
    if (has('name')) payload.name = clean(patch.name) || 'New Deal';

    if (has('serviceLine')) payload.serviceLine = clean(patch.serviceLine) || null;
    if (has('service_line')) payload.serviceLine = clean(patch.service_line) || null;

    if (has('marketSegment')) payload.marketSegment = clean(patch.marketSegment) || null;
    if (has('market_segment')) payload.marketSegment = clean(patch.market_segment) || null;
    if (has('renewablesSegment')) payload.marketSegment = clean(patch.renewablesSegment) || null;

    if (has('opportunityType')) payload.opportunityType = clean(patch.opportunityType) || null;
    if (has('opportunity_type')) payload.opportunityType = clean(patch.opportunity_type) || null;

    if (has('stage')) payload.stage = frontendStageToBackend(patch.stage);

    if (has('forecastCategory')) payload.forecastCategory = frontendForecastToBackend(patch.forecastCategory);
    if (has('forecast_category')) payload.forecastCategory = frontendForecastToBackend(patch.forecast_category);

    if (has('probability')) payload.forecastProbability = Number(patch.probability || 0) || 0;
    if (has('forecastProbability')) payload.forecastProbability = Number(patch.forecastProbability || 0) || 0;

    if (has('expectedCloseDate')) {
      const raw = clean(patch.expectedCloseDate);
      payload.expectedCloseDate = raw ? new Date(raw).toISOString() : null;
    }
    if (has('expected_close_date')) {
      const raw = clean(patch.expected_close_date);
      payload.expectedCloseDate = raw ? new Date(raw).toISOString() : null;
    }

    if (has('scopeNotes')) payload.notes = clean(patch.scopeNotes) || null;
    if (has('notes')) payload.notes = clean(patch.notes) || null;

    if (has('ownerUserId')) payload.ownerUserId = clean(patch.ownerUserId) || null;

    if (has('annualRevenue')) payload.annualRevenue = Number(patch.annualRevenue || 0) || 0;
    if (has('arr')) payload.arr = Number(patch.arr || 0) || 0;
    if (has('totalEstimatedRevenue')) payload.totalEstimatedRevenue = Number(patch.totalEstimatedRevenue || 0) || 0;
    if (has('oneTimeRevenue')) payload.oneTimeRevenue = Number(patch.oneTimeRevenue || 0) || 0;
    if (has('ctsPercent')) payload.ctsPercent = Number(patch.ctsPercent || 0) || 0;
    if (has('marginPercent')) payload.marginPercent = Number(patch.marginPercent || 0) || 0;
  ...
  async function deleteOpportunityViaApi(opportunityId) {
    const response = await backendFetch(`/api/opportunities/${opportunityId}`, {
      method: 'DELETE',
    });

    if (!response.ok) {
      throw new Error(`Opportunity delete failed: ${response.status}`);
    }
  ...
  async function updateOpportunityViaApi(opportunityId, patch = {}, currentRecord = {}) {
    const payload = buildOpportunityPatchPayload(patch);
  ...
  (async () => {
    try {
      const data = await fetchOpportunitiesFromApi();

      if (Array.isArray(data) && data.length > 0) {
        setOpportunities(data);
      } else {
        const raw = window.localStorage.getItem('naes-crm-opportunity-list');
        const localData = sanitizeOpportunities(
          safeParseStoredJson(raw, initialOpportunities)
        );

        setOpportunities(localData);
        console.warn('Backend returned zero opportunities, using localStorage fallback');
      }
    } catch (error) {
      console.warn('Backend opportunity fetch failed, using localStorage fallback', error);
  ...
  const [userAccounts, setUserAccounts] = useState(() =>
    buildInitialUserAccounts(buildDefaultUserProfile()).map((item) => buildUserAccountDraft(item))
  );
  const [cognitoCallbackStatus, setCognitoCallbackStatus] = useState('idle');
  const [authChecked, setAuthChecked] = useState(false);

  useEffect(() => {
    let cancelled = false;

    async function finishCognitoCallback() {
      const params = new URLSearchParams(window.location.search);
      const code = params.get('code');

      if (!code) return;

      setCognitoCallbackStatus('loading');
      ...
      try {
        ...
        if (!exchangeResponse.ok || !exchangeJson?.ok) {
          throw new Error(exchangeJson?.error || 'cognito_exchange_failed');
        }

        const email =
          exchangeJson?.data?.email ||
          exchangeJson?.data?.id_claims?.email ||
          exchangeJson?.data?.access_claims?.email ||
          exchangeJson?.data?.claims?.email ||
          null;

        const sub =
          exchangeJson?.data?.sub ||
          exchangeJson?.data?.id_claims?.sub ||
          exchangeJson?.data?.access_claims?.sub ||
          exchangeJson?.data?.claims?.sub ||
          null;

        const mapResponse = await fetch('/api/auth/cognito/map-user', {
          method: 'POST',
          credentials: 'include',
          headers: { 'Content-Type': 'application/json' },
        ...
        if (!mapResponse.ok || !mapJson?.ok) {
          throw new Error(mapJson?.error || 'cognito_map_user_failed');
        }
        ...
        if (!cancelled) {
          setCognitoCallbackStatus('error');
        }
      }
    }

    finishCognitoCallback();
  ...
  useEffect(() => {
    ...
    async function checkSessionGate() {
      ...
      try {
        ...
        window.location.href = 'https://auth.naestechnologiescrm.com/oauth2/authorize?response_type=code&client_id=vkj3d1jc939nseosicn5v6phl&redirect_uri=https://www.naestechnologiescrm.com/&scope=openid+email+profile';
      } catch (error) {
        console.error('Session gate failed', error);
        window.location.href = 'https://auth.naestechnologiescrm.com/oauth2/authorize?response_type=code&client_id=vkj3d1jc939nseosicn5v6phl&redirect_uri=https://www.naestechnologiescrm.com/&scope=openid+email+profile';
      }
    }
  ...
  const effectiveDevHeaderUserId = null;
  const effectiveUserRole = userProfile?.role || 'Associate';
  ...
  const safeActivePage =
    (activePage === 'New Deal Intake') ? 'New Deal Intake' :
    routeInfo.isOpportunities ? 'Opportunities' :
  ...
  useEffect(() => {
    ...
    async function hydrateUsersFromBackend() {
      try {
        const backendUsers = await fetchUsersFromApi();
    ...
    async function hydrateCurrentUserProfile() {
      try {
        ...
        if (!cancelled && json?.ok && json?.data) {
          setUserProfile(mapBackendUserToFrontend(json.data));
        }
      } catch (error) {
        console.warn('Could not hydrate current user profile', error);
      }
    }
  ...
  React.useEffect(() => {
    try {
      if (Array.isArray(opportunities) && opportunities.length > 0) {
        ...
        console.warn('Skipped writing empty opportunities list to localStorage');
      }
      } else {
        console.warn('Skipped saving empty opportunities list to localStorage');
      }
    } catch (error) {
    ...
    async function hydrateOpportunitiesFromBackend() {
      try {
        ...
        if (!cancelled && Array.isArray(backendOpportunities) && backendOpportunities.length) {
          console.log('🚫 backend disabled, keeping local opportunities');
        }
      } catch (error) {
...
window.deleteAccount = async function(accountId) {
  await deleteAccountViaApi(accountId);
  setAccountList(prev => prev.filter(a => a.id !== accountId));
};

async function saveAccountFromV2(draft) {
  const normalized = {
    ...draft,
    estimatedBuildingCount: draft.estimatedBuildingCount || draft.siteCount || '',
    totalMw: draft.totalMw || draft.estimatedMw || '',
    estimatedSquareFootage: draft.estimatedSquareFootage || draft.estimatedSqft || '',
    portfolioType: draft.portfolioType || draft.revenueTier || '',
    generalFootprintRegion: draft.generalFootprintRegion || draft.region || '',
    primaryAccountOwner: draft.primaryAccountOwner || draft.owner || '',
    owner: draft.owner || draft.primaryAccountOwner || ''
  };

  const rawAccountId = normalized.id || normalized._backend?.id || normalized.accountId;
  const accountId = String(rawAccountId || '').startsWith('account-') ? '' : rawAccountId;
  const saved = accountId
    ? await updateAccountViaApi(accountId, normalized)
    : await createAccountViaApi(normalized);

  setAccountList((prev) => {
    const exists = prev.some((account) => account.id === saved.id);
    return exists
      ? prev.map((account) => account.id === saved.id ? { ...account, ...saved } : account)
      : [saved, ...prev];
  });
...
async function saveEditedAccount() {
    ...
    let nextRecord = await updateAccountViaApi(accountDetailId, newAccountForm);
...
async function saveNewOpportunity() {
  ...
  const newOpportunity = {
    id: newId,
    name: String(newOpportunityForm.name || 'New Deal').trim(),
...
function openOpportunityDetail(opportunityId) {
    const nextPath = `/opportunities-v2/${opportunityId}`;
    pushPath(nextPath);
    setRoutePath(nextPath);
    setActivePage('Opportunities & Analytics');
  }
  ...
  async function saveOpportunityPatch(opportunityId, patch) {
    ...
    try {
      ...
      nextRecord = {
        ...
        account:
          ...
          || backendRecord?.account?.name
          || (typeof backendRecord?.account === 'string' ? backendRecord.account : '')
          || '',
        account_name:
          currentRecord.account_name
          || backendRecord?.account?.name
          || backendRecord.account_name
          || '',
        owner_full_name:
          [backendRecord?.owner?.firstName, backendRecord?.owner?.lastName].filter(Boolean).join(' ').trim()
          || backendRecord.owner_full_name
          || fallbackRecord.owner_full_name
          || '',
        ownerName:
          [backendRecord?.owner?.firstName, backendRecord?.owner?.lastName].filter(Boolean).join(' ').trim()
          || backendRecord.ownerName
          || fallbackRecord.ownerName
          || '',
        owner:
          [backendRecord?.owner?.firstName, backendRecord?.owner?.lastName].filter(Boolean).join(' ').trim()
          || backendRecord.owner
          || fallbackRecord.owner
          || '',
        ownerUserId:
          String(backendRecord?._backend?.ownerUserId || backendRecord?.ownerUserId || fallbackRecord.ownerUserId || '').trim(),
        owner_team_name:
          String(backendRecord?.owner?.teamName || backendRecord.owner_team_name || fallbackRecord.owner_team_name || '').trim(),
        primary_contact_name:
          currentRecord.primary_contact_name
          || [backendRecord?.primaryContact?.firstName, backendRecord?.primaryContact?.lastName].filter(Boolean).join(' ').trim()
          || backendRecord.primary_contact_name
          || '',
        };
      } catch (error) {
    ...
    async function deleteOpportunityLocal(opportunityId) {
      try {
        await deleteOpportunityViaApi(opportunityId);
        const refreshed = await fetchOpportunitiesFromApi();
        setOpportunities(refreshed);
      } catch (error) {
        console.warn('Could not delete opportunity from backend', error);
        window.alert('Delete failed. The opportunity was not removed from the backend.');
      }
    }
  ...
  if (!authChecked && cognitoCallbackStatus === 'idle') {
    return null;
  }

  if (cognitoCallbackStatus === 'loading') {
    return (
      <div style={{ minHeight: '100vh', display: 'grid', placeItems: 'center', background: '#f5f7fb', color: '#123B42', fontFamily: 'Arial, sans-serif' }}>
        <div style={{ fontSize: '18px', fontWeight: 700 }}>Signing you in...</div>
      </div>
    );
  }

  if (cognitoCallbackStatus === 'error') {
    return (
      <div style={{ minHeight: '100vh', display: 'grid', placeItems: 'center', background: '#f5f7fb', color: '#123B42', fontFamily: 'Arial, sans-serif', padding: '24px' }}>
        <div style={{ maxWidth: '640px', textAlign: 'center' }}>
          <div style={{ fontSize: '22px', fontWeight: 800, marginBottom: '12px' }}>Sign-in could not be completed</div>
          <div style={{ fontSize: '14px', lineHeight: 1.6, color: '#5F6F72' }}>
            The CRM received the Cognito callback but could not finish session setup. Refresh once and try again.
          </div>
        </div>
      </div>
    );
      ...
      <div style={{ display: 'grid', gridTemplateRows: 'minmax(0, 1fr) 46px', height: '100%', overflow: 'hidden' }}>
                        ...
                        <button
                          key={item === 'Signal' ? (
                                <span style={{ display: 'inline-flex', alignItems: 'center', gap: '8px' }}>
                                  <span>{item}</span>
                                  <span style={{
                                    border: '1px solid rgba(11,103,113,0.25)',
                                    background: '#e0f2fe',
                                    color: '#075985',
                                    borderRadius: '999px',
                                    padding: '2px 7px',
        ...
        <main style={{ minWidth: 0, minHeight: 0, height: '100%', overflowY: 'auto', overflowX: 'hidden' }}>
          ...
          <div style={{ padding: '32px' }}>
            {safeActivePage === 'Opportunities & Analytics' || safeActivePage === 'Opportunities'
              ? (
                  selectedOpportunity
                      ? <NewDealV2
                            key={selectedOpportunity.id}
                            mode="edit"
                            initialDeal={selectedOpportunity}
                            users={userAccounts}
                            onBack={returnToOpportunitiesOverview}
                            onCreate={async (payload) => {
                              const patch = {
                                ...selectedOpportunity,
                                ...payload,
                                service_line: payload.serviceLine || selectedOpportunity.service_line,
                                market_segment: payload.marketSegment || selectedOpportunity.market_segment,
                                account_name: payload.accountName || payload.account || selectedOpportunity.account_name || '',
                                owner_full_name: payload.ownerName || payload.owner || selectedOpportunity.owner_full_name || '',
                                estimated_square_footage: payload.estimated_square_footage ?? payload.sqft ?? selectedOpportunity.estimated_square_footage,
                                price_per_sqft: payload.price_per_sqft ?? payload.pricePerSqft ?? selectedOpportunity.price_per_sqft,
                                term_years: payload.term_years ?? payload.termYears ?? selectedOpportunity.term_years,
                                total_mwdc: payload.total_mwdc ?? payload.mw ?? selectedOpportunity.total_mwdc,
                                capacity_basis: payload.capacity_basis || payload.basis || 'MWDC',
                                calculated_revenue: payload.calculated_revenue ?? payload.annualRevenue ?? selectedOpportunity.calculated_revenue,
                                amount_total: payload.amount_total ?? payload.totalEstimatedRevenue ?? selectedOpportunity.amount_total,
                                amount_estimated: payload.amount_estimated ?? payload.totalEstimatedRevenue ?? selectedOpportunity.amount_estimated,
                                weighted_revenue: Number(payload.weighted_revenue || 0),
                              };
                              await saveOpportunityPatch(selectedOpportunity.id, patch);
                              const refreshed = await fetchOpportunitiesFromApi();
setOpportunities(refreshed);
returnToOpportunitiesOverview();
                            }}
                          />
                    : <OpportunitiesV2
                        opportunities={opportunities}
                        onOpenOpportunity={openOpportunityDetail}
                        onDeleteOpportunity={deleteOpportunityLocal}
                        onStartNewDeal={() => {
                          setActivePage('New Deal Intake');
                        }}
                      />
                )
              : safeActivePage === 'New Deal Intake'
                ? <NewDealV2 users={userAccounts} onCreate={async (payload) => {
    const fallbackRecord = {
      id: `opp-${Date.now()}`,
      name: payload.name || 'New Deal',
      service_line: payload.serviceLine || 'Renewables',
      market_segment: payload.marketSegment || payload.serviceLine || '',
      opportunity_type: payload.opportunityType || 'New Customer',
      stage: payload.stage || '0 Prospecting',
      forecast_category: payload.forecastCategory || 'Pipeline',
      probability: Number(payload.forecastProbability || payload.probability || 50),
      calc_year1_total: Number(payload.annualRevenue || 0),
      calc_arr_total: Number(payload.arr || payload.annualRevenue || 0),
      amount_total: Number(payload.totalEstimatedRevenue || payload.annualRevenue || 0),
      amount_estimated: Number(payload.totalEstimatedRevenue || payload.annualRevenue || 0),
      calculated_revenue: Number(payload.annualRevenue || 0),
      weighted_revenue: Number(payload.totalEstimatedRevenue || 0) * (Number(payload.forecastProbability || 50) / 100),
      estimated_earnings_pct: 37,
      estimated_cts_pct: 63,
      account_name: payload.accountName || payload.account || '',
      owner_full_name: payload.ownerName || payload.owner || '',
        estimated_square_footage: payload.estimated_square_footage ?? payload.sqft ?? selectedOpportunity.estimated_square_footage,
        price_per_sqft: payload.price_per_sqft ?? payload.pricePerSqft ?? selectedOpportunity.price_per_sqft,
        term_years: payload.term_years ?? payload.termYears ?? selectedOpportunity.term_years,
        total_mwdc: payload.total_mwdc ?? payload.mw ?? selectedOpportunity.total_mwdc,
        ...
        basis: payload.basis,
          ...
          scanFrequency: payload.scanFrequency || '',
          expected_close_date: payload.expected_close_date || payload.expectedCloseDate || '',
          deal_entry_date: payload.deal_entry_date || dtNow(),
          stage_updated_date: payload.stage_updated_date || dtNow(),
          omDescription: payload.omDescription || '',
          omBasis: payload.omBasis || '',
          omAnnual: payload.omAnnual || 0,
          omOneTime: payload.omOneTime || 0,
          omTerm: payload.omTerm || 0,
          omConfidence: payload.omConfidence || '',
          omScopeClarity: payload.omScopeClarity || '',
          omNotes: payload.omNotes || '',
      created_at: dtNow(),
      updated_at: dtNow()
    };

    let nextOpportunity = fallbackRecord;
    try {
      const backendOpportunity = await createOpportunityViaApi(payload, fallbackRecord);
      nextOpportunity = { ...fallbackRecord, ...backendOpportunity };
    } catch (error) {
      console.warn('Could not create NewDealV2 opportunity in backend, using local fallback', error);
    }pushPath('/opportunities-v2-dashboard');
      setRoutePath('/opportunities-v2-dashboard');
      setActivePage('Opportunities & Analytics');
  }} onBack={() => { pushPath('/opportunities-v2-dashboard'); setRoutePath('/opportunities-v2-dashboard'); pushPath('/opportunities-v2-dashboard');
      setRoutePath('/opportunities-v2-dashboard');
      setActivePage('Opportunities & Analytics'); }} />
              ...
              ? <OpportunitiesV2
                  opportunities={opportunities.filter(o =>
                    String(o?.owner_full_name || o?.owner || '').trim() === String(userProfile?.fullName || '').trim()
                  )}
                  titleOverride="My Command Center"
                  onOpenOpportunity={openOpportunityDetail}
                  onDeleteOpportunity={deleteOpportunityLocal}
                  onStartNewDeal={() => setActivePage('New Deal Intake')}
                />
              ...
              ? (
                showEditAccountForm
                  ...
                  ? <AccountDetailPage
                    ...
                    />
                  : <AccountsV2
                      initialAccount={signalAccountSeed}
                      accounts={accountList}
                      onCreateAccount={saveNewAccount}
                      onUpdateAccount={saveAccountFromV2}
...
/>
              ...
              ? (
                showNewAccountForm
                  ? <NewAccountPage
                      ...
                      onCreate={(draft) => {
                        setAccountList(prev => [draft, ...prev]);
                                          }}
                      onCancelNewAccount={cancelNewAccount}
                    />
                  : showEditAccountForm
                  ? <NewAccountPage
                      ...
                      onCreate={(draft) => {
                        setAccountList(prev =>
                          prev.map(a =>
                            a.id === accountDetailId ? { ...a, ...draft } : a
                          )
                        );
                                          }}
                      onCancelNewAccount={cancelEditAccount}
              ...
              : safeActivePage === 'Account Intelligence'
                ? <AccountsV2
                      initialAccount={signalAccountSeed}
                    accounts={accountList}
                    onCreateAccount={saveAccountFromV2}
                    onUpdateAccount={saveAccountFromV2}
                    forceOpenNewAccount={showNewAccountForm}
                    onClearForceOpenNewAccount={() => setShowNewAccountForm(false)}
                    onOpenAccount={(accountId) => {
                      openAccountDetail(accountId);
                    }}
                  />
                  ...
                  <div style={{ display: 'grid', gap: '18px' }}>
                    <div style={{
                      ...
                      <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginTop: '18px' }}>
                        {['Dashboard', 'Apollo Search', 'Campaigns', 'Event History', 'Upload List'].map((tab) => {
                          ...
                          <SignalChartErrorBoundary>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: '18px', marginBottom: '18px' }}>
                              <div style={signalExecutiveChartCard}>
                                <h4 style={{ margin: 0, color: '#123B42', fontSize: '16px', fontWeight: 850 }}>Apollo Revenue Potential by Service</h4>
                                <p style={{ margin: '4px 0 16px', color: '#5F6F72', fontSize: '11px', lineHeight: 1.45 }}>Modeled potential revenue by service type across imported Signal targets.</p>
                              ...
                              <div style={signalExecutiveChartCard}>
                                ...
                                <p style={{ margin: '4px 0 16px', color: '#5F6F72', fontSize: '11px', lineHeight: 1.45 }}>Estimated commercial value by CRM service fit.</p>
                                <div style={{ height: '310px' }}>
                                  <RechartsResponsiveContainer width="100%" height="100%">
                              ...
                              <div style={signalExecutiveChartCard}>
                                ...
                                <p style={{ margin: '4px 0 16px', color: '#5F6F72', fontSize: '11px', lineHeight: 1.45 }}>Target count by Signal qualification tier.</p>
...
Top Signal Targets</h4>
                                <p style={{ margin: '4px 0 16px', color: '#5F6F72', fontSize: '11px', lineHeight: 1.45 }}>Highest-ranked accounts by computed Signal Score.</p>
                                <div style={{ height: '280px' }}>
                                  <RechartsResponsiveContainer width="100%" height="100%">
                            ...
                            <div style={{ ...signalExecutiveChartCard, margin: '22px 0 22px' }}>
                              <h4 style={{ margin: 0, color: '#123B42', fontSize: '16px', fontWeight: 850 }}>
                                Signal Event Summary
                              </h4>
                              <p style={{ margin: '4px 0 16px', color: '#5F6F72', fontSize: '11px', lineHeight: 1.45 }}>
                                Safe-mode execution telemetry across preview, simulated send, and future engagement events.
                              </p>
                              <div style={{ height: '260px', marginTop: '10px' }}>
                                <RechartsResponsiveContainer width="100%" height="100%">
                                  <RechartsBarChart data={signalEventRows}>
                                    <RechartsCartesianGrid strokeDasharray="3 3" stroke="#E5E7EB" />
                                    <RechartsXAxis dataKey="event" tick={{ fontSize: 10, fill: '#465A69' }} axisLine={{ stroke: '#9CA3AF' }} tickLine={false} />
                                    <RechartsYAxis allowDecimals={false} tick={{ fontSize: 10, fill: '#465A69' }} axisLine={{ stroke: '#9CA3AF' }} tickLine={false} />
                                    <RechartsTooltip />
                                    <RechartsBar dataKey="count" fill="#0B6771" radius={[8, 8, 0, 0]} barSize={42} />
                                  </RechartsBarChart>
                                </RechartsResponsiveContainer>
                              </div>
                            </div>

                            <div style={{ ...signalExecutiveChartCard, margin: '0 0 24px' }}>
                              <h4 style={{ margin: 0, color: '#123B42', fontSize: '15px', fontWeight: 850 }}>Signal Campaign Funnel</h4>
                              <p style={{ margin: '4px 0 16px', color: '#5F6F72', fontSize: '11px', lineHeight: 1.45 }}>Contact progression through the Signal pipeline.</p>
                              <div style={{ height: '260px' }}>
                                <RechartsResponsiveContainer width="100%" height="100%">
                                  <RechartsBarChart data={signalFunnelRows} layout="vertical" margin={{ top: 10, right: 24, left: 60, bottom: 8 }}>
                                    <RechartsCartesianGrid strokeDasharray="3 3" stroke="#E5E7EB" />
                                    <RechartsXAxis type="number" allowDecimals={false} tick={{ fontSize: 10, fill: '#465A69' }} axisLine={{ stroke: '#9CA3AF' }} tickLine={false} />
                                    <RechartsYAxis type="category" dataKey="stage" width={100} tick={{ fontSize: 10, fill: '#123B42', fontWeight: 700 }} axisLine={{ stroke: '#9CA3AF' }} tickLine={false} />
                                    <RechartsTooltip />
                                    <RechartsBar dataKey="value" fill="#0B6771" radius={[0, 8, 8, 0]} barSize={26} />
                                  </RechartsBarChart>
                                </RechartsResponsiveContainer>
                              </div>
                            </div>
                            ...
                            <div style={{ ...signalExecutiveChartCard, margin: '0 0 24px' }}>
                              <h4 style={{ margin: 0, color: '#123B42', fontSize: '15px', fontWeight: 850 }}>Signal Event Trend</h4>
                              <p style={{ margin: '4px 0 16px', color: '#5F6F72', fontSize: '11px' }}>Daily activity trend across previews, sends, opens, clicks, and suppressions.</p>
                              <div style={{ height: '280px' }}>
                                <RechartsResponsiveContainer width="100%" height="100%">
                                  <RechartsComposedChart data={signalEventTrendRows}>
                                    <RechartsCartesianGrid strokeDasharray="3 3" stroke="#E5E7EB" />
                                    <RechartsXAxis dataKey="day" tick={{ fontSize: 10 }} />
                                    <RechartsYAxis allowDecimals={false} tick={{ fontSize: 10 }} />
                                    <RechartsTooltip />
                                    <RechartsLegend />
                                    <RechartsBar dataKey="previews" fill="#0B6771" />
                                    <RechartsLine type="monotone" dataKey="sends" stroke="#3F8F66" />
                                    <RechartsLine type="monotone" dataKey="opens" stroke="#F59E0B" />
                                    <RechartsLine type="monotone" dataKey="clicks" stroke="#2563EB" />
                                    <RechartsLine type="monotone" dataKey="suppressions" stroke="#991B1B" />
                                  </RechartsComposedChart>
                                </RechartsResponsiveContainer>
                              </div>
                            </div>
...
<SignalMiniChartBoundary>
                            <div style={{ display: 'grid', gridTemplateColumns: '1.35fr 0.9fr', gap: '18px', marginBottom: '18px' }}>
                              <div style={signalExecutiveChartCard}>
                                ...
                                <p style={{ margin: '4px 0 16px', color: '#5F6F72', fontSize: '11px', lineHeight: 1.45 }}>Projected Signal revenue build from imported targets over the modeled campaign period.</p>
                              ...
                              <div style={signalExecutiveChartCard}>
                                ...
                                <p style={{ margin: '4px 0 16px', color: '#5F6F72', fontSize: '11px', lineHeight: 1.45 }}>Revenue value already moved into the controlled intake queue versus remaining Signal potential.</p>
                                <div style={{ height: '300px' }}>
                                  <RechartsResponsiveContainer width="100%" height="100%">
                    ...
                    ) : signalOutreachTab === 'Apollo Search' ? (
                      <div style={{ display: 'grid', gridTemplateColumns: '280px minmax(0, 1fr) 340px', gap: '16px' }}>
                        <div style={{ border: '1px solid #d7e4d8', borderRadius: '22px', background: '#ffffff', padding: '16px', boxShadow: '0 14px 32px rgba(16,24,40,0.05)' }}>
                          <div style={{ fontSize: '10px', fontWeight: 900, letterSpacing: '0.18em', textTransform: 'uppercase', color: '#6b7f76' }}>Apollo Filters</div>
                          <h3 style={{ margin: '6px 0 14px', color: '#111827' }}>Search Criteria</h3>

                          {[
                            ['Company / Keyword', 'solar, logistics, rooftop...'],
                            ['Industry', 'Renewables, Logistics...'],
                            ['HQ Region', 'AZ, TX, CA, NV...'],
                            ['Employee Count', '250+'],
                            ['Revenue Range', '$100M+'],
                            ['Technology / Asset Signal', 'PV, rooftop, cold storage...']
                          ].map(([label, placeholder]) => (
                            <label key={label} style={{ display: 'grid', gap: '6px', marginBottom: '12px', color: '#445f54', fontSize: '12px', fontWeight: 850 }}>
                              {label}
                              <input disabled placeholder={placeholder} style={{ border: '1px solid #d7e4d8', borderRadius: '12px', padding: '10px 12px', background: '#f8fafc', color: '#64748b' }} />
                            </label>
                          ))}

                          <div style={{ border: '1px solid #e0f2fe', background: '#f0f9ff', color: '#075985', borderRadius: '14px', padding: '12px', fontSize: '12px', fontWeight: 800, lineHeight: 1.45 }}>
                            API mode will run through CRM backend only. No Apollo credentials will be exposed in the browser.
                          </div>
                        </div>
                        ...
                        <div style={{ border: '1px solid #d7e4d8', borderRadius: '22px', background: '#ffffff', padding: '16px', boxShadow: '0 14px 32px rgba(16,24,40,0.05)' }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px', alignItems: 'center', marginBottom: '14px' }}>
                            <div>
                              <div style={{ fontSize: '10px', fontWeight: 900, letterSpacing: '0.18em', textTransform: 'uppercase', color: '#6b7f76' }}>Apollo Search</div>
                              <h3 style={{ margin: '6px 0 0', color: '#111827' }}>Company & contact discovery</h3>
                            </div>
                            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                              <span style={{ border: '1px solid #d7e4d8', background: '#f8fbf8', color: '#315348', borderRadius: '999px', padding: '6px 10px', fontSize: '12px', fontWeight: 850 }}>API: Backend Preview</span>
                              <span style={{ border: '1px solid #e0f2fe', background: '#f0f9ff', color: '#075985', borderRadius: '999px', padding: '6px 10px', fontSize: '12px', fontWeight: 850 }}>{apolloSearchStatus}</span>
                            </div>
                          </div>

                          <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) auto', gap: '10px', marginBottom: '14px' }}>
                            <input
                              value={apolloSearchTerm}
                              onChange={(event) => setApolloSearchTerm(event.target.value)}
                              placeholder="Search Apollo companies, contacts, industries, signals..."
                              style={{ border: '1px solid #d7e4d8', borderRadius: '14px', padding: '12px 14px', background: '#ffffff', fontSize: '13px' }}
                            />
                            <button
                              type="button"
                              onClick={runApolloPreviewSearch}
                              style={{ border: '1px solid #0B6771', background: '#0B6771', color: '#fff', borderRadius: '14px', padding: '0 18px', fontWeight: 900, cursor: 'pointer' }}
                            >
                              Search
                            </button>
                          </div>
                          ...
                          {apolloSearchError ? (
                            <div style={{ border: '1px solid #fecaca', background: '#fff7f7', color: '#991b1b', borderRadius: '14px', padding: '10px 12px', fontSize: '12px', fontWeight: 800, marginBottom: '12px' }}>
                              {apolloSearchError}
                            </div>
                          ) : null}

                          <div style={{ display: 'grid', gap: '12px' }}>
                            {activeApolloRecords.map((record) => (
                              <div key={record.company} style={{ border: '1px solid #e3ece4', borderRadius: '18px', padding: '14px', background: '#ffffff' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px', alignItems: 'flex-start' }}>
                                  <div>
                                    <strong style={{ color: '#111827', fontSize: '15px' }}>{record.company}</strong>
                                    <div style={{ marginTop: '4px', color: '#64748b', fontSize: '12px' }}>{record.industry} · {record.hq} · {record.employees} employees · {record.revenue}</div>
                                  </div>
                                  <span style={{ border: '1px solid #bae6fd', background: '#f0f9ff', color: '#075985', borderRadius: '999px', padding: '5px 9px', fontSize: '11px', fontWeight: 900 }}>{record.confidence}</span>
                                </div>

                                <p style={{ margin: '10px 0 0', color: '#334155', fontSize: '13px', lineHeight: 1.45 }}>{record.buyingSignal}</p>

                                <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginTop: '12px' }}>
                                  <span style={{ border: '1px solid #d7e4d8', background: '#f8fbf8', color: '#315348', borderRadius: '999px', padding: '5px 9px', fontSize: '11px', fontWeight: 850 }}>Fit: {record.serviceFit}</span>
                                  <span style={{ border: '1px solid #d7e4d8', background: '#f8fbf8', color: '#315348', borderRadius: '999px', padding: '5px 9px', fontSize: '11px', fontWeight: 850 }}>{record.contacts} contacts</span>
                                  <span style={{ border: '1px solid #d7e4d8', background: '#f8fbf8', color: '#315348', borderRadius: '999px', padding: '5px 9px', fontSize: '11px', fontWeight: 850 }}>Potential: {record.estimatedRevenue}</span>
                                </div>
                                  ...
                                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', gap: '10px', marginTop: '14px' }}>
                                    <button
                                      ...
                                      onClick={() => {
                                          const prospect = {
                                            ...
                                            businessType: record.industry,
                                            region: record.hq,
                                            hq: record.hq,
                                            serviceFit: [record.serviceFit].filter(Boolean),
                                            marketSegment: record.serviceFit,
                                            signalInsight: record.buyingSignal,
                                            buyingSignal: record.buyingSignal,
                                        ...
                                        seedSignalAccountIntake(prospect);
                                      }}
                                      style={{ border: '1px solid #b9d8c4', background: '#eaf5ee', color: '#153d2f', borderRadius: '12px', padding: '12px 10px', fontSize: '12px', fontWeight: 900, cursor: 'pointer' }}
                                    >
                                    ...
                                    <button
                                      ...
                                      onClick={() => {
                                        ...
                                        seedSignalContactIntake(prospect);
                                      }}
                                      style={{ border: '1px solid #b7dde5', background: '#eef9fb', color: '#0B6771', borderRadius: '12px', padding: '12px 10px', fontSize: '12px', fontWeight: 900, cursor: 'pointer' }}
                                    >
                                      Contacts
                                    </button>
                                    ...
                                    <button
                                      ...
                                      onClick={() => {
                                        ...
                                        seedSignalDealIntake(prospect);
                                      }}
                                      style={{ border: '1px solid #b9d8f0', background: '#eef6ff', color: '#1476b8', borderRadius: '12px', padding: '12px 10px', fontSize: '12px', fontWeight: 900, cursor: 'pointer' }}
                                    >
                                      New Deal
                                    </button>
                                  </div>
                        ...
                        <div style={{ border: '1px solid #d7e4d8', borderRadius: '22px', background: '#ffffff', padding: '16px', boxShadow: '0 14px 32px rgba(16,24,40,0.05)' }}>
                          <div style={{ fontSize: '10px', fontWeight: 900, letterSpacing: '0.18em', textTransform: 'uppercase', color: '#6b7f76' }}>Selected Record Preview</div>
                          <h3 style={{ margin: '6px 0 14px', color: '#111827' }}>Enrichment Pipeline</h3>

                          {[
                            ['1', 'Search', 'Find companies and candidate contacts.'],
                            ['2', 'Enrich', 'Pull company/contact details server-side.'],
                            ['3', 'Normalize', 'Map Apollo result into Signal prospect schema.'],
                            ['4', 'Score', 'Apply Signal scoring and revenue model.'],
                            ['5', 'Queue', 'Send to controlled CRM intake queue.']
                          ].map(([step, title, body]) => (
                            <div key={step} style={{ display: 'grid', gridTemplateColumns: '30px minmax(0, 1fr)', gap: '10px', borderTop: '1px solid #edf2ee', padding: '12px 0' }}>
                              <div style={{ width: '28px', height: '28px', borderRadius: '999px', background: '#0B6771', color: '#fff', display: 'grid', placeItems: 'center', fontSize: '12px', fontWeight: 900 }}>{step}</div>
                              <div>
                                <strong style={{ color: '#111827', fontSize: '13px' }}>{title}</strong>
                                <div style={{ color: '#64748b', fontSize: '12px', marginTop: '3px', lineHeight: 1.4 }}>{body}</div>
                              </div>
                            </div>
                          ))}

                          <div style={{ marginTop: '14px', border: '1px dashed #9fb9ad', borderRadius: '16px', padding: '14px', background: '#f8fbf8', color: '#315348', fontSize: '12px', fontWeight: 800, lineHeight: 1.45 }}>
                            Next backend endpoint: /api/signal/apollo/search. Credentials stay on server or AWS Secrets Manager.
                          </div>
                        </div>
                      </div>
                      ...
                      ) : signalOutreachTab === 'Campaigns' ? (
                        <div style={{ border: '1px solid #d7e4d8', borderRadius: '22px', background: '#fff', padding: '20px' }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px', alignItems: 'flex-start', marginBottom: '14px' }}>
                            <div>
                              <div style={{ fontSize: '10px', fontWeight: 900, letterSpacing: '0.18em', textTransform: 'uppercase', color: '#6b7f76' }}>Campaign Service</div>
                              <h3 style={{ margin: '6px 0 4px', color: '#111827' }}>Signal Campaigns</h3>
                              <p style={{ margin: 0, color: '#5e786d' }}>
                                Campaigns are live. Safe Mode allows full send simulation with tracking and event logging. Live Mode unlocks once deliverability (SPF/DKIM/DMARC) is verified.
                              </p>
                            </div>
                            <button
                              type="button"
                              onClick={loadSignalCampaigns}
                              style={{ border: '1px solid #0B6771', background: '#0B6771', color: '#fff', borderRadius: '999px', padding: '8px 12px', fontSize: '12px', fontWeight: 850, cursor: 'pointer' }}
                            >
                              Refresh
                            </button>
                          </div>

                          <div style={{ marginBottom: '16px', border: '1px solid #d7e4d8', borderRadius: '16px', padding: '16px', background: '#f8fbf8' }}>
                            <div style={{ fontWeight: 900, marginBottom: '10px', color: '#111827' }}>Template Engine</div>
                            <input
                              value={campaignSubject}
                              onChange={(e) => setCampaignSubject(e.target.value)}
                              placeholder="Subject"
                              style={{ width: '100%', padding: '10px', borderRadius: '10px', border: '1px solid #d7e4d8', marginBottom: '10px' }}
                            />
                            <textarea
                              value={campaignBody}
                              onChange={(e) => setCampaignBody(e.target.value)}
                              rows={6}
                              style={{ width: '100%', padding: '10px', borderRadius: '10px', border: '1px solid #d7e4d8' }}
                            />
                            <div style={{ marginTop: '10px', color: '#64748b', fontSize: '12px', fontWeight: 800 }}>
                              Variables: {'{{first_name}}'} {'{{company}}'} {'{{service_fit}}'} {'{{signal_insight}}'}
                            </div>
                          </div>

                          <div style={{ marginBottom: '16px', border: '1px solid #d7e4d8', borderRadius: '16px', background: '#f8fbf8', padding: '14px' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px', alignItems: 'flex-start', marginBottom: '12px' }}>
                              <div>
                                <div style={{ color: '#123B42', fontSize: '12px', fontWeight: 900, letterSpacing: '0.08em', textTransform: 'uppercase' }}>Send Readiness</div>
                                <button
                                  type="button"
                                  onClick={loadSignalReadiness}
                                  style={{
                                    marginLeft: '8px',
                                    border: '1px solid #d7e4d8',
                                    background: '#ffffff',
                                ...
                                <div style={{ color: '#5e786d', fontSize: '12px', marginTop: '4px' }}>
                                  Live sending remains locked until deliverability controls are verified.
                                </div>
                              </div>
                              <span style={{
                                ...
                                {signalReadiness.live_ready ? 'Live Ready' : 'Safe Mode'}
                            ...
                            <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap', marginBottom: '12px' }}>
                              {[
                                ...
                                const active = signalSendMode === mode;
                                const locked = mode === 'live' && !signalReadiness.live_ready;
                                return (
                                  <button
                                    key={mode}
                                    type="button"
                                    disabled={locked}
                                    onClick={() => setSignalSendMode(mode)}
                                    title={locked ? 'Live mode is locked until all readiness checks pass.' : label}
                                    style={{
                                      border: active ? '1px solid #0B6771' : '1px solid #d7e4d8',
                                      background: active ? '#0B6771' : '#ffffff',
                                      color: locked ? '#94a3b8' : (active ? '#ffffff' : '#315348'),
                                      borderRadius: '999px',
                                      padding: '7px 11px',
                                      fontSize: '11px',
                                      fontWeight: 900,
                              ...
                              <span style={{ color: '#64748b', fontSize: '11px', fontWeight: 800 }}>
                                Selected send mode: {signalSendMode === 'live' ? 'Live' : 'Safe'}
                              </span>
                            </div>

                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0, 1fr))', gap: '10px' }}>
                              {(signalReadiness.checks || []).map((check) => (
                                <div key={check.key || check.label} title={check.detail} style={{ border: '1px solid #d7e4d8', borderRadius: '14px', background: '#ffffff', padding: '12px' }}>
                                  <div style={{ display: 'flex', alignItems: 'center', gap: '7px', marginBottom: '6px' }}>
                                    <span style={{
                                      ...
                                      ? '#0B6771'
                                      : (check.key === 'live_send' ? '#991b1b' : '#d97706'),
                                      display: 'inline-block'
                                    }} />
                                    <strong style={{ color: '#111827', fontSize: '12px' }}>{check.label}</strong>
                                  </div>
                                  <div style={{ color: '#64748b', fontSize: '11px', lineHeight: 1.4 }}>{check.detail}</div>
                                </div>
                              ))}
                            </div>
                          </div>
                          ...
                          {signalCampaigns.length === 0 ? (
                            <div style={{ border: '1px dashed #9fb9ad', borderRadius: '18px', background: '#f8fbf8', padding: '28px', textAlign: 'center', color: '#445f54', fontWeight: 800 }}>
                              No campaigns yet. Create one from the Intake Queue.
                            </div>
                          ) : (
                            <div style={{ display: 'grid', gap: '12px' }}>
                              {signalCampaigns.map((campaign) => (
                                <div key={campaign.id} style={{ border: '1px solid #d7e4d8', borderRadius: '18px', padding: '16px', background: '#ffffff' }}>
                                  <div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px', alignItems: 'flex-start' }}>
                                    <div>
                                      <strong style={{ color: '#111827', fontSize: '16px' }}>{campaign.name}</strong>
                                      <div style={{ marginTop: '4px', color: '#64748b', fontSize: '12px' }}>
                                        {campaign.segment || 'General'} · {campaign.status || 'draft'} · {(campaign.contacts || []).length} contacts
                                      </div>
                                    </div>
                                    <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                                      <span style={{ border: '1px solid #fed7aa', background: '#fff7ed', color: '#9a3412', borderRadius: '999px', padding: '5px 9px', fontSize: '11px', fontWeight: 900 }}>
                                        Safe Mode Active
                                      </span>
                                      <button
                                        type="button"
                                        onClick={() => runSignalSendPreview(campaign.id)}
                                        style={{ border: '1px solid #0B6771', background: '#0B6771', color: '#fff', borderRadius: '999px', padding: '6px 10px', fontSize: '11px', fontWeight: 900, cursor: 'pointer' }}
                                      >
                                        Preview Send <span title="Preview Send renders the final personalized email payload, including subject, body, open pixel, and tracked links. It does not create a send event and does not send email." style={{ marginLeft: '6px', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: '14px', height: '14px', borderRadius: '999px', background: 'rgba(255,255,255,0.18)', fontSize: '10px', fontWeight: 900 }}>i</span>
                                      </button>
                                      <button
                                        type="button"
                                        onClick={() => runSignalSimulatedSend(campaign.id)}
                                        style={{ border: '1px solid #065f46', background: '#065f46', color: '#fff', borderRadius: '999px', padding: '6px 10px', fontSize: '11px', fontWeight: 900, cursor: 'pointer' }}
                                      >
                                        Simulate Send <span title="Simulate Send creates safe-mode sent_simulated events directly from the UI. It is used to test analytics, events, and funnel reporting. No email leaves the system." style={{ marginLeft: '6px', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: '14px', height: '14px', borderRadius: '999px', background: 'rgba(255,255,255,0.18)', fontSize: '10px', fontWeight: 900 }}>i</span>
                                      </button>
                                      <button
                                        type="button"
                                        onClick={() => runSignalSafeSend(campaign.id)}
                                        style={{ border: '1px solid #111827', background: '#111827', color: '#fff', borderRadius: '999px', padding: '6px 10px', fontSize: '11px', fontWeight: 900, cursor: 'pointer' }}
                                      >
                                        {signalSendMode === 'live' ? 'Send Campaign (Live)' : 'Send Campaign (Safe)'} <span title="Send Campaign uses the backend send engine. In Safe Mode it prepares send payloads, checks suppressions, logs results, and updates telemetry. Live delivery remains blocked until readiness checks pass." style={{ marginLeft: '6px', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: '14px', height: '14px', borderRadius: '999px', background: 'rgba(255,255,255,0.18)', fontSize: '10px', fontWeight: 900 }}>i</span>
                                      </button>
<div style={{marginTop:'6px',color:'#64748b',fontSize:'11px',fontWeight:800}}>Safe Mode: no emails sent • full tracking + events logged</div>
                                    </div>
                                  </div>

                                  <div style={{ marginTop: '14px', display: 'grid', gap: '8px' }}>
                                    
                                    <div style={{ marginTop: '14px', border: '1px solid #e5e7eb', borderRadius: '12px', padding: '12px', background: '#ffffff' }}>
                                      <div style={{ fontSize: '11px', fontWeight: 900, color: '#0B6771', marginBottom: '8px' }}>
                                      ...
                                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', gap: '8px' }}>
                                        <input placeholder="First name"
                                          ...
                                          style={{ border: '1px solid #d7e4d8', borderRadius: '8px', padding: '8px' }}
                                        />
                                        ...
                                        <input placeholder="Company"
                                          ...
                                          style={{ border: '1px solid #d7e4d8', borderRadius: '8px', padding: '8px' }}
                                        />
                                        <input placeholder="Service fit"
                                          value={manualContact.service_fit}
                                          onChange={(e) => setManualContact({ ...manualContact, service_fit: e.target.value })}
                                          style={{ border: '1px solid #d7e4d8', borderRadius: '8px', padding: '8px' }}
                                        />
                                        <input placeholder="Signal insight"
                                          value={manualContact.signal_insight}
                                          onChange={(e) => setManualContact({ ...manualContact, signal_insight: e.target.value })}
                                          style={{ border: '1px solid #d7e4d8', borderRadius: '8px', padding: '8px' }}
                                        />
                                      </div>
                                      ...
                                      <button
                                        type="button"
                                        onClick={() => addManualContactToCampaign(campaign.id)}
                                        style={{ marginTop: '10px', border: '1px solid #0B6771', background: '#0B6771', color: '#fff', borderRadius: '10px', padding: '8px 12px', fontWeight: 900, cursor: 'pointer' }}
                                      >
                                        Add Contact
                                      </button>
                                    </div>
...
{(campaign.contacts || []).length === 0 ? (
                                      <div style={{ color: '#64748b', fontSize: '13px' }}>No contacts added yet.</div>
                                    ) : (
                                      (campaign.contacts || []).map((contact) => (
                                        <div key={`${contact.email || contact.name}-${contact.company}`} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '10px', borderTop: '1px solid #edf2ee', paddingTop: '9px', color: '#334155', fontSize: '12px' }}>
                                          <div><strong>{contact.name || `${contact.first_name || ''} ${contact.last_name || ''}`.trim() || 'Unknown Contact'}</strong><br />{contact.title || 'No title'}</div>
                                          <div>{contact.company || 'No company'}<br />{contact.email || 'No email'}</div>
                                          <div>{contact.service_fit || contact.serviceFit || 'No fit'}<br />{contact.source || 'Signal'}</div>
                                        </div>
                                      ))
                                    )}
                                  </div>

                                  {(campaign.contacts || []).slice(0, 1).map((c, i) => (
                                    <div key={`template-preview-${campaign.id}-${i}`} style={{ marginTop: '14px', padding: '12px', background: '#f8fbf8', border: '1px solid #d7e4d8', borderRadius: '12px' }}>
                                      <div style={{ color: '#315348', fontSize: '12px', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.08em' }}>Live Template Preview</div>
                                      <div style={{ fontWeight: 900, marginTop: '8px', color: '#111827' }}>{renderTemplate(campaignSubject, c)}</div>
                                      <pre style={{ whiteSpace: 'pre-wrap', margin: '8px 0 0', color: '#334155', fontSize: '12px', fontFamily: 'inherit' }}>{renderTemplate(campaignBody, c)}</pre>
                                    </div>
                                  ))}

                                  {sendResultSummaries[campaign.id] ? (
                                    <div style={{ marginTop: '14px', border: '1px solid #d7e4d8', background: '#f8fbf8', borderRadius: '14px', padding: '12px' }}>
                                      <div style={{ color: '#123B42', fontSize: '12px', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.08em' }}>Last Send Result</div>
                                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', gap: '10px', marginTop: '10px' }}>
                                        <div style={{ background: '#ffffff', border: '1px solid #d7e4d8', borderRadius: '12px', padding: '10px' }}>
                                          <div style={{ color: '#64748b', fontSize: '11px', fontWeight: 800 }}>Mode</div>
                                          <strong style={{ color: '#111827', fontSize: '14px' }}>{sendResultSummaries[campaign.id].mode}</strong>
                                        </div>
                                        <div style={{ background: '#ffffff', border: '1px solid #d7e4d8', borderRadius: '12px', padding: '10px' }}>
                                          <div style={{ color: '#64748b', fontSize: '11px', fontWeight: 800 }}>Prepared</div>
                                          <strong style={{ color: '#0B6771', fontSize: '18px' }}>{sendResultSummaries[campaign.id].sent_count}</strong>
                                        </div>
                                        <div style={{ background: '#ffffff', border: '1px solid #d7e4d8', borderRadius: '12px', padding: '10px' }}>
                                          <div style={{ color: '#64748b', fontSize: '11px', fontWeight: 800 }}>Blocked</div>
                                          <strong style={{ color: sendResultSummaries[campaign.id].blocked_count ? '#991b1b' : '#0B6771', fontSize: '18px' }}>{sendResultSummaries[campaign.id].blocked_count}</strong>
                                        </div>
                                      </div>
                                      {(sendResultSummaries[campaign.id].blocked || []).length ? (
                                        <div style={{ marginTop: '10px', color: '#991b1b', fontSize: '11px', lineHeight: 1.45 }}>
                                          Blocked: {(sendResultSummaries[campaign.id].blocked || []).map((item) => item.email).join(', ')}
                                        </div>
                                      ) : null}
                                    </div>
                                  ) : null}

                                  {(sendPreviewPayloads[campaign.id] || []).length ? (
                                    <div style={{ marginTop: '14px', border: '1px solid #bae6fd', background: '#f0f9ff', borderRadius: '14px', padding: '12px' }}>
                                      <div style={{ color: '#075985', fontSize: '12px', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.08em' }}>Send Preview Payload</div>
                                      {(sendPreviewPayloads[campaign.id] || []).map((payload) => (
                                        <div key={payload.message_id} style={{ marginTop: '10px', background: '#fff', border: '1px solid #d7e4d8', borderRadius: '12px', padding: '12px' }}>
                                          <div style={{ color: '#334155', fontSize: '12px' }}><strong>To:</strong> {payload.to || 'No email'}</div>
                                          <div style={{ color: '#111827', fontSize: '13px', fontWeight: 900, marginTop: '6px' }}>{payload.subject || 'No subject'}</div>
                                          <pre style={{ whiteSpace: 'pre-wrap', margin: '8px 0 0', color: '#334155', fontSize: '12px', fontFamily: 'inherit' }}>{payload.body || ''}</pre>
                                            {(payload.tracked_links || []).length ? (
                                              <div style={{ marginTop: '10px', borderTop: '1px solid #e2e8f0', paddingTop: '10px' }}>
                                                <div style={{ color: '#075985', fontSize: '11px', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.08em' }}>Tracked Links</div>
                                                {(payload.tracked_links || []).map((link, idx) => (
                                                  <div key={`${payload.message_id}-tracked-link-${idx}`} style={{ marginTop: '6px', color: '#334155', fontSize: '11px', lineHeight: 1.45 }}>
                                                    <strong>{link.label || 'Link'}</strong><br />
                                                    Destination: {link.url || 'n/a'}<br />
                                                    Tracking: {link.click_url || 'n/a'}
                                                  </div>
                                                ))}
                                              </div>
                                            ) : null}
                                            {payload.open_pixel_url ? (
                                              <div style={{ marginTop: '10px', color: '#64748b', fontSize: '11px', lineHeight: 1.45 }}>
                                                <strong>Open Pixel:</strong> {payload.open_pixel_url}
                                              </div>
                                            ) : null}
                                        </div>
                                      ))}
                      ...
                      ) : (signalOutreachTab === 'Events' || signalOutreachTab === 'Event History') ? (
                        <div style={{ border: '1px solid #d7e4d8', borderRadius: '22px', background: '#fff', padding: '20px' }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px', alignItems: 'center', marginBottom: '14px' }}>
                            <div>
                              <div style={{ fontSize: '10px', fontWeight: 900, letterSpacing: '0.18em', textTransform: 'uppercase', color: '#6b7f76' }}>Signal Event Stream</div>
                              <h3 style={{ margin: '6px 0 4px', color: '#111827' }}>Events</h3>
                              <p style={{ margin: 0, color: '#5e786d' }}>Safe-mode event tracking for previews, sends, opens, clicks, replies, bounces, complaints, and conversions.</p>
                            </div>
                            <button
                              type="button"
                              onClick={loadSignalEvents}
                              style={{ border: '1px solid #0B6771', background: '#0B6771', color: '#fff', borderRadius: '999px', padding: '8px 12px', fontSize: '12px', fontWeight: 850, cursor: 'pointer' }}
                            >
                              Refresh
                            </button>
                          ...
                          {signalEvents.length === 0 ? (
                            <div style={{ border: '1px dashed #9fb9ad', borderRadius: '18px', background: '#f8fbf8', padding: '28px', textAlign: 'center', color: '#445f54', fontWeight: 800 }}>
                              No Signal events logged yet. Open Campaigns and click Preview Send.
                            </div>
                          ) : (
                            <div style={{ display: 'grid', gap: '10px' }}>
                              {signalEvents.map((event) => (
                                <div key={event.id} style={{ border: '1px solid #d7e4d8', borderRadius: '14px', padding: '12px', background: '#ffffff', display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '10px', color: '#334155', fontSize: '12px' }}>
                                  <div><strong style={{ color: '#111827' }}>{event.event_type}</strong><br />{event.created_at}</div>
                                  <div>Campaign: {event.campaign_id || 'n/a'}<br />Message: {event.message_id || 'n/a'}</div>
                                  <div>Contact: {event.contact_id || 'n/a'}<br />Source: {event.source || 'signal-ui'}</div>
                                </div>
                              ))}
                            </div>
                          )}
                        </div>
                      ) : signalOutreachTab === 'Suppressions' ? (
                        <div style={{ border: '1px solid #d7e4d8', borderRadius: '22px', background: '#fff', padding: '20px' }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px', alignItems: 'center', marginBottom: '14px' }}>
                            <div>
                              <div style={{ fontSize: '10px', fontWeight: 900, letterSpacing: '0.18em', textTransform: 'uppercase', color: '#6b7f76' }}>Deliverability Guardrail</div>
                              <h3 style={{ margin: '6px 0 4px', color: '#111827' }}>Suppressions</h3>
                              <p style={{ margin: 0, color: '#5e786d' }}>Contacts blocked from outbound sends due to manual suppression, bounce, complaint, or unsubscribe events.</p>
                            </div>
                            <button
                              type="button"
                              onClick={loadSignalSuppressions}
                              style={{ border: '1px solid #0B6771', background: '#0B6771', color: '#fff', borderRadius: '999px', padding: '8px 12px', fontSize: '12px', fontWeight: 850, cursor: 'pointer' }}
                            >
                              Refresh
                            </button>
                          ...
                          <div style={{ marginBottom: '16px', border: '1px solid #d7e4d8', borderRadius: '16px', background: '#f8fbf8', padding: '14px' }}>
                            <div style={{ color: '#123B42', fontSize: '12px', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '10px' }}>Manual Suppression</div>
                            <div style={{ display: 'grid', gridTemplateColumns: 'minmax(220px, 1fr) minmax(220px, 1fr) auto', gap: '10px', alignItems: 'center' }}>
                              <input
                                value={suppressionEmail}
                                onChange={(event) => setSuppressionEmail(event.target.value)}
                                placeholder="email@company.com"
                                style={{ border: '1px solid #d7e4d8', borderRadius: '12px', padding: '10px 12px', background: '#ffffff', fontSize: '12px' }}
                              />
                              <input
                              ...
                              <button
                                type="button"
                                onClick={createSignalSuppression}
                                disabled={!suppressionEmail.trim()}
                                style={{
                                  border: '1px solid #991b1b',
                                  background: suppressionEmail.trim() ? '#991b1b' : '#f1f5f9',
                                  color: suppressionEmail.trim() ? '#ffffff' : '#94a3b8',
                                  borderRadius: '999px',
                                  padding: '9px 12px',
                                  fontSize: '12px',
                                  fontWeight: 900,
                                  cursor: suppressionEmail.trim() ? 'pointer' : 'not-allowed',
                                  whiteSpace: 'nowrap'
                                }}
                              >
                                Suppress Email
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: 'minmax(180px, 240px) auto minmax(0, 1fr)', gap: '10px', alignItems: 'center', marginTop: '10px' }}>
                              <select
                                value={suppressionEventType}
                                onChange={(event) => setSuppressionEventType(event.target.value)}
                                style={{ border: '1px solid #d7e4d8', borderRadius: '12px', padding: '10px 12px', background: '#ffffff', fontSize: '12px' }}
                              >
                              ...
                              <button
                                type="button"
                                onClick={simulateSuppressionEvent}
                                disabled={!suppressionEmail.trim()}
                                style={{
                                  border: '1px solid #92400e',
                                  background: suppressionEmail.trim() ? '#fff7ed' : '#f1f5f9',
                                  color: suppressionEmail.trim() ? '#92400e' : '#94a3b8',
                                  borderRadius: '999px',
                                  padding: '9px 12px',
                                  fontSize: '12px',
                                  fontWeight: 900,
                              ...
                              <div style={{ color: '#64748b', fontSize: '11px', lineHeight: 1.4 }}>
                                Test bounce, complaint, or unsubscribe automation. These events automatically create active suppressions.
                              </div>
                            </div>
                          </div>

                          {signalSuppressions.length === 0 ? (
                            <div style={{ border: '1px dashed #9fb9ad', borderRadius: '18px', background: '#f8fbf8', padding: '28px', textAlign: 'center', color: '#445f54', fontWeight: 800 }}>
                              No suppressions currently active or recorded.
                            </div>
                          ) : (
                            <div style={{ overflowX: 'auto' }}>
                              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
                                ...
                                <tbody>
                                  {signalSuppressions.map((item) => (
                                    <tr key={item.id || item.email} style={{ borderBottom: '1px solid #edf2ee' }}>
                                      <td style={{ padding: '11px 8px', fontWeight: 800, color: '#111827' }}>{item.email}</td>
                                      <td style={{ padding: '11px 8px' }}>{item.type || 'manual'}</td>
                                      <td style={{ padding: '11px 8px', color: '#475569' }}>{item.reason || 'n/a'}</td>
                                      <td style={{ padding: '11px 8px' }}>{item.source || 'signal'}</td>
                                      <td style={{ padding: '11px 8px' }}>
                                        <span style={{
                                          border: item.active === false ? '1px solid #d7e4d8' : '1px solid #fecaca',
                                          background: item.active === false ? '#f8fafc' : '#fff7f7',
                                          color: item.active === false ? '#64748b' : '#991b1b',
                                          borderRadius: '999px',
                                          padding: '4px 8px',
                                          fontWeight: 900
                                        }}>
                                      ...
                                      <td style={{ padding: '11px 8px', color: '#64748b' }}>{item.updated_at || item.created_at || 'n/a'}</td>
                                      <td style={{ padding: '11px 8px' }}>
                                        <button
                                          type="button"
                                          disabled={item.active === false}
                                          onClick={() => removeSignalSuppression(item.email)}
                                          style={{
                                            border: item.active === false ? '1px solid #d7e4d8' : '1px solid #0B6771',
                                            background: item.active === false ? '#f8fafc' : '#0B6771',
                                            color: item.active === false ? '#94a3b8' : '#ffffff',
                                            borderRadius: '999px',
                                            padding: '6px 10px',
                                            fontSize: '11px',
                                            fontWeight: 900,
                      ...
                      ) : (signalOutreachTab === 'Import CSV' || signalOutreachTab === 'Upload List') ? (
                      <div style={{ border: '1px solid #d7e4d8', borderRadius: '22px', background: '#fff', padding: '20px' }}>
                      ...
                      <div style={{ border: '1px solid #d7e4d8', borderRadius: '22px', background: '#fff', padding: '20px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px', alignItems: 'flex-start', marginBottom: '14px' }}>
                          <div>
                            ...
                            <p style={{ margin: 0, color: '#5e786d' }}>
                              Review queued Signal prospects before seeding Account, Contact, or Deal intake.
          <div style={{ marginTop: '16px', display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
            <select
              value={selectedCampaignId}
              onChange={(e) => setSelectedCampaignId(e.target.value)}
              style={{ padding: '8px', borderRadius: '10px', border: '1px solid #d7e4d8' }}
            >
              <option value="">Select Campaign</option>
              {signalCampaigns.map((c) => (
            ...
            <button
              onClick={createSignalCampaign}
              style={{ padding: '8px 12px', background: '#0B6771', color: '#fff', borderRadius: '10px', border: 'none', fontWeight: 800 }}
            >
              Create
            </button>

            <button
              onClick={addQueueToCampaign}
              style={{ padding: '8px 12px', background: '#065f46', color: '#fff', borderRadius: '10px', border: 'none', fontWeight: 800 }}
            >
              Add Queue → Campaign
            </button>
          </div>
 No backend records are created until the existing CRM intake forms are saved.
                            </p>
                          </div>
                          <button
                            type="button"
...
: safeActivePage === 'Settings'
              ? (
                <SettingsPage
                  ...
                  onSaveUserProfile={saveUserProfileViaApi}
              ...
              ? (
                <UserAccountsPage
                  ...
                  onCreateUserAccount={createUserViaApi}
                  onUpdateUserAccount={updateUserViaApi}
                />
              )
