  ...
  if (!opportunity) {
    return (
      ...
      <div className="mt-6 space-y-4">
        <SectionCard title="Executive Pipeline Analytics">
          <div className="grid gap-4 md:grid-cols-2">

            <div>
              <h4 className="text-sm font-bold mb-2">Weighted TCV by Stage</h4>
              {typeof renderVerticalBarChart === 'function' &&
                renderVerticalBarChart([
                  { label: opportunity.stage, value: weightedTcv || 0 }
                ], (v) => formatCurrency(v))
              }
            </div>

            <div>
              <h4 className="text-sm font-bold mb-2">Quarterly Forecast</h4>
              {typeof renderHorizontalBarChart === 'function' &&
                renderHorizontalBarChart([
                  { label: opportunity.expectedClose || 'Forecast', value: weightedRevenue || 0 }
                ], (v) => formatCurrency(v))
              }
            </div>
